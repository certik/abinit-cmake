...
&  ' bottom of conduction [eV] ',cbot*Ha_eV
  ...
  allocate(eigent(nbnds*nkibz*nsppol),nbandt(nkibz*nsppol))
  ...
  write(msg,'(a,f9.5)')' fermi : calling newocc with fixmom = ',fixmom
  call wrtout(std_out,msg,'COLL')
  allocate(occt(nbnds*nkibz*nsppol),doccdet(nbnds*nkibz*nsppol))
...
&      '  Occupation number of band ',ib,' for kpt ',ik,' spin ',is,ch10,&
  ...
  deallocate(eigent,nbandt,occt,doccdet)
 end if
