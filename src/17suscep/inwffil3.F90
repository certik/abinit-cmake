...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2008 ABINIT group (GMR,AR,MB)
...
!scalars
 ...
 integer :: band_index,fform0,formeig,iband,ierr,ikpt,ikpt0,ikptsp_old,isppol
 integer :: isppol0,istwf_k,master,me,nband1,nband_k,nkpt_eff,npw1,npw_k
 integer :: nspinor1,rdwr,restart,spaceworld,tim_rwwf
...
!  (from ikptsp_old+1 to ikpt+(isppol-1)*nkpt-1)
   ...
   tim_rwwf=0
!  DEBUG
!  write(6,*)' inwffil3 : will call WffReadEig '
!  ENDDEBUG
...
! Prepare transmission of eigen
  band_index=0
  allocate(buffer1(mband*nkpt*nsppol))
  do isppol=1,nsppol
   do ikpt=1,nkpt
    nband_k=nband(ikpt+(isppol-1)*nkpt)
