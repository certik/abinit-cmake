...
!!
!! NOTES
!! the interface of this routine is mandatory for some other routines
!! If you happen to modify it, please update src/defs/defs_dyson.F90
!!
!!
!! PARENTS
!!      xcacfd
...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 ...
 use interfaces_17suscep, except_this_one => acfd_dyson
...
!scalars
 ...
 integer,intent(in) :: occopt,option,suskxcrs
...
!arrays
 ...
 real(dp),intent(in) :: gmet(3,3),gsq(npwdiel),rhor(nfft,nspden),rprimd(3,3)
...
!scalars
 ...
 integer :: npwtestmax
 real(dp) :: deccc,di,dimax,dr,drmax,dummy,energy_nozero,lambda,lambda_step
 logical :: khxcalloc,kxcpassed,kxcsaved,testflag
...
!Compute the Hartree kernel with a cut-off in real space.
 ...
 allocate(krpa(npwdiel))


!DEBUG
!write(6,*)' acfd_dyson : call k_rpa '
!call flush(6)
!ENDDEBUG
...
!the RPA kernel...
 ...
 if (kxcpassed) then
  allocate(kxcdsave(npwdiel,nspden,nspden))
  do isp2 = 1,nspden
   do isp1 = 1,nspden
    do ipw = 1,npwdiel
...
!Allocate memory for the kernel.
 ...
 if (ikernel == 0) allocate(khxcdiag(npwdiel))

 khxcalloc = (ikernel == 1).and.((.not.kxcpassed).or.kxcsaved)
 if (khxcalloc) allocate(khxc(2,npwdiel,nspden,npwdiel,nspden))

 if (nkxc > 0) then
  allocate(kxcgold(2,nfft,nkxc))
  kxcgold(:,:,:) = 0._dp
 end if
...
!Set-up abscissas and weights for the coupling-strength integration.
 ...
 allocate(lambda_quad(nlambda),weight_quad(nlambda))

 call getlambda(idyson,lambda_quad,nlambda,weight_quad)

!Allocate memory.

 allocate(susd_nks(npwdiel))
 allocate(susd_isc(npwdiel))

 if (option > 0) then
  allocate(susd_tmp(npwdiel))
  allocate(sus_gabs(npw_tiny))
  allocate(sus_gavg(npw_tiny))
  allocate(sus_gdir(npw_tiny,3))
 end if

 if (option > 1) then
  allocate(energy(npw_tiny))
  allocate(decuc(npw_tiny))
  allocate(decuc_quad(npw_tiny,nlambda))
  allocate(deccc_quad(nlambda))
 end if
...
!  Using the bare Coulomb interaction:
   ...
   call geteexc_uc(energy,energy_nozero,gsq,ig_tiny,igsq_tiny,npwdiel,npw_tiny,susd_isc)
   decuc(:) = -0.5_dp*energy(:)/two_pi !factor -1/(2*pi) from fluctuation-dissipation theorem.

!  Using the cut-off Coulomb interaction:

   call geteexc_cc(energy,energy_nozero,gsq,npwdiel,npw_tiny,0,rcut_coulomb,susd_isc)
...
!DEBUG
!write(6,*)' acfd_dyson : will solve the Dyson equation'
!call flush(6)
!ENDDEBUG
   ...
   if(suskxcrs==0)then
    ...
    select case (ikhxc)
     ...
     case (ikhxc_ALDA, ikhxc_BPG, ikhxc_EOK2)
      ...
      allocate(kxcg(2,nfft,nkxc))
      allocate(kxcgdiff(2,nfft))
      allocate(rhor_lambda(nfft,nspden))
      ...
      deallocate(kxcg,kxcgdiff,rhor_lambda)
    ...
    if(ikhxc_ALDA/=ikhxc)then
     write(6,*)' acfd_dyson : suskxcrs==1 presently implemented only for ikhxc=ALDA '
     stop
    end if
    if(nspden/=1)then
     write(6,*)' acfd_dyson : suskxcrs==1 presently implemented only for nspden=1 '
     stop
    end if
    khxcdiag(:) = krpa(:)*lambda
...
!Free memory.
 ...
 if (ikernel == 0) deallocate(khxcdiag)
 if (khxcalloc) deallocate(khxc)

 if (nkxc > 0) deallocate(kxcgold)

 deallocate(krpa)
 deallocate(lambda_quad)
 deallocate(weight_quad)
 deallocate(susd_nks)
 deallocate(susd_isc)
 ...
 if (option > 1) then
  deallocate(energy)
  deallocate(decuc)
  deallocate(decuc_quad)
  deallocate(deccc_quad)
 end if
...
& (ikhxc == ikhxc_EOK1).or.(ikhxc == ikhxc_EOK2)).and.(nspden == 1)) then
  ...
  allocate(khxcdiag(npwdiel),energy(npwdiel),susd_tmp(4*npwdiel),sus_gavg(3*npwdiel))
...
! Restore the lower triangle (including the diagonal) of susmat.
  ...
  deallocate(khxcdiag,energy,susd_tmp,sus_gavg)
