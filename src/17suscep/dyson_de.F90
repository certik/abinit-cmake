...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2008 ABINIT group (MF, XG, YMN).
...
#if defined T3E
   call CHEMM('l','l',ndim,ndim,c1,susmat,ndim,&
...
#if defined T3E
   call CGEMM('n','n',ndim,ndim,ndim,c1,susmat,ndim,&
&   kernel_full,ndim,c0,susmat_P_kernel,ndim)
#else
...
!TODO : column by column multiplication to avoid the use of susmat_new.
 ...
 allocate(susmat_new(2,npwdiel,nspden,npwdiel,nspden))
 susmat_new(:,:,:,:,:) = susmat(:,:,:,:,:)

 if (.true.) then

#if defined T3E
  call CHEMM('r','l',ndim,ndim,c1,susmat_new,ndim,&
...
#if defined T3E
  call CGEMM('n','n',ndim,ndim,ndim,c1,susmat_P_kernel,ndim,&
...
&  susmat_new,ndim,c1,susmat,ndim)
 ...
 deallocate(susmat_new,susmat_P_kernel)
