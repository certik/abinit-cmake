...
#ifdef VMS
!DEC$ ATTRIBUTES ALIAS:'ZGEMV' :: zgemv
#endif
...
!Allocate memory.
 ...
 allocate(drho_next(2,npwdiel))
 allocate(drho_this(2,npwdiel))
 allocate(drho_zero(2,npwdiel))
 allocate(susmat_P_kernel(2,npwdiel))
...
#if defined T3E
    call CGEMV('n',npwdiel,npwdiel,c1,susmat,npwdiel,susmat_P_kernel,1,&
&    c1,drho_next,1)
#else
...
! write(6,'(a,i6,a,1x,2(1x,e14.6),1x,a,i3)') &
! &      ' ipw=',     ipw1,&
...
! End loop over ipw1.
 ...
 deallocate(drho_next)
 deallocate(drho_this)
 deallocate(drho_zero)
 deallocate(susmat_P_kernel)

!DEBUG
!write(6,*) ' %dyson_sc: end and exit'
!ENDDEBUG
