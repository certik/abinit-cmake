...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2008 ABINIT group (DCA, FRD, XG, GMR, PT, SC)
...
! Fill up the matrix in packed storage
  ...
  if(numproj/=0)then
   ...
   allocate (uu(numproj,numproj),zz(2,numproj,numproj))

   if (numproj > 1) then
#if defined T3E
    call CHPEV(jobz,uplo,numproj,ap,ww,zz,numproj,work1,rwork1,info)
#else
...
&    yp1j(1),ypnj(1),ppspl(:,2,4,1),work)
