...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2008 ABINIT group (DCA, XG, GMR, FrD, FJ, MT)
...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 ...
 use interfaces_13psp, except_this_one => psp5in
...
!scalars
 ...
 integer,intent(in) :: n1xccc,pspcod,pspso,useylm
...
!wfll(:,1),...,wfll(:,4)=reference config. wavefunctions
 allocate (vloc(mmax),vpspll(mmax,mpsang))
...
!  DEBUG
!  write(6,*) 'END OF READING PSP',ll,'OK'
!  ENDDEBUG
  end do
 else

! --SPIN-ORBIT
  allocate(vpspll_so(mmax,mpsang))
  read (tmp_unit,*) ll
...
!DEBUG
!write(6,*) 'VLOC=',vloc(1),vloc(2),vloc(3)
!write(6,*) 'VLOC=',vloc(4),vloc(5),vloc(6)
!ENDDEBUG
...
!DEBUG
!write(6,*) 'HAMMAN RADIAL GRID r1 and al',r1,al
!write(6,*) 'rad(1)=',rad(1)
!write(6,*) 'rad(10)=',rad(10)
!write(6,*) 'rad(100)=',rad(100)
!ENDDEBUG
...
!DEBUG
!write(6,*)' psp5in : before nonlocal corrections '
!write(6,*)' psp5in : lloc, lmax = ',lloc,lmax
!if(.true.)stop
!ENDDEBUG
...
!    DEBUG
!    write(6,*) 'l=',ll,'OK CHECK'
!    ENDDEBUG
     read (tmp_unit,*) wfll(:,ipsang)
!    DEBUG
!    write(6,*) 'WF READED',ll,'OK'
!    write(6,*) 'WF',ipsang-1,wfll(1,ipsang),&
...
! Compute KB form factors and fit splines
  allocate(ekb_tmp(mpssoang,max(nso,mproj)))
  allocate(ffspl_tmp(mqgrid,2,mpssoang,max(nso,mproj)))
  ekb_tmp(:,:)=0.d0

  allocate(ekb_sr(mpsang),ffspl_sr(mqgrid,2,mpsang))
...
&  qgrid,rad,vloc,vpspll,wfll)
  ...
  if (pspso/=0) then
   allocate (ekb_so(mpsang),ffspl_so(mqgrid,2,mpsang))
...
&   qgrid,rad,vloc,vpspll_so,wfll_so)
   ...
   do ipsang=2,lmax+1
    if((ekb_sr(ipsang)*ekb_so(ipsang))<0.0) then
     print*, 'BIG PROBLEM WITH THE SPIN ORBIT IN PSP5NL'
     stop
    end if
   end do
...
&      /(2.d0*ipsang-1)
   ...
   deallocate(ekb_so,ffspl_so,vpspll_so,wfll_so)
...
&      '  Action : check your pseudopotential input file.'
     ...
     read (tmp_unit,*) wfll(:,ipsang)
!    DEBUG
!    write(6,*) 'WF second',ipsang-1,wfll(1,ipsang),&
!    & wfll(2,ipsang),wfll(3,ipsang)
!    ENDDEBUG
    else
     wfll(:,ipsang)=0.0d0
...
! Convert ekb and ffspl
  ...
  deallocate(ekb_tmp,ffspl_tmp,wfll)

! end of if concerning lloc
 end if

 deallocate(vpspll,rad,vloc)

!DEBUG
!write(6,*)' psp5in : vlspl(1,2)= ',vlspl(1,2)
!ENDDEBUG
