...
!Arrays
!scalars
 integer :: ia,iaph3d,ii,il,ilmn,iplex,ipwshft,ispinor
 real(dp) :: scale,wt
 logical :: parity
!arrays
...
!Inits
 ...
 if (paw_opt>=3) then
  ...
  if (choice>1) allocate(dgxdtfacs_(2,ndgxdtfac,nlmn))
 end if
...
! End loop on spinors
 ...
 if (paw_opt>=3) then
  deallocate(gxfacs_);if (choice>1) deallocate(dgxdtfacs_)
 end if
