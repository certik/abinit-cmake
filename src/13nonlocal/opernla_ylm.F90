...
!!         5 : compute projected scalars and derivatives wrt wave vector k
...
!!  vect(2,npw*nspinor)=starting vector in reciprocal space
!!
!! OUTPUT
...
!Local variables-------------------------------
!no_abirules
 integer :: choice_,gama,gamb,gamc,gamd,ia,iaph3d,ierr,il,ilmn,ipw,ipw0,ipwshft,ishift,ispinor,jpw,mu
...
!   --------------------------------------------------------------------
    ...
    if (signs==1) then
     allocate(scalarr(npw,10),scalari(npw,10))
     do ilmn=1,nlmn
      il=mod(indlmn(1,ilmn),4);parity=(mod(il,2)==0)
      scale=wt;if (il>1) scale=-scale
