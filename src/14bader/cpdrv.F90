...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 ...
 use interfaces_14bader, except_this_one => cpdrv
...
!Local variables ------------------------------
!scalars
 integer :: iat,iatinit,ii,inxat,inxcell,ipos,iposinit,ires,jj,kk,ll,nb,nb_now
 integer :: nedge,nn,nstep,nvrts,nvs
 real(dp),parameter :: tol3=1.0d-3
 real(dp) :: chg,diff1,diff2,diff3,dist,prj,rtdiff,ss,sum,tt0,wall
 logical :: srch=.false.
!arrays
 integer :: ibat(nnpos*natom),idn(2),inatm(nnpos*natom),incell(nnpos*natom)
 ...
 real(dp),allocatable :: sortguide(:),vrts(:,:)
!no_abirules
...
!  varying the fastest
   allocate(sortguide(nbcp),indexcp(nbcp),cp_tmp(nbcp))
   do ii=3,1,-1
!   DEBUG
!   write(6,*)' cpdrv : sort on index ii=',ii
!   ENDDEBUG
...
&    bcp(jj)%rr(:),bcp(jj)%ev(:),bcp(jj)%ev(1)+bcp(jj)%ev(2)+bcp(jj)%ev(3)
   ...
   deallocate(cp_tmp,indexcp,sortguide)
...
!  varying the fastest
   allocate(sortguide(nrcp),indexcp(nrcp),cp_tmp(nrcp))
   do ii=3,1,-1
!   DEBUG
!   write(6,*)' cpdrv : sort on index ii=',ii
!   ENDDEBUG
...
!  varying the fastest
   allocate(sortguide(nccp),indexcp(nccp),cp_tmp(nccp))
   do ii=3,1,-1
    do jj=1,nccp
!    Try to be platform-independent. Might need a larger tolerance.
...
&    ccp(jj)%rr(:),ccp(jj)%ev(:),ccp(jj)%ev(1)+ccp(jj)%ev(2)+ccp(jj)%ev(3)
   ...
   deallocate(sortguide,indexcp,cp_tmp)
