...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2008 ABINIT group (FDortu,MVeithen)
...
!Open the '.eps' file for write
 write(6,'(a,a)') 'Opening file ', filnam(2)
 open(18, file=filnam(2))
!Open the phonon energies file
 write(6,'(a,a)') 'Opening file ', filnam(3)
 open(19, file=filnam(3))
 if(filnam(4)/='no') then
! Open the displacements file
  write(6,'(a,a)') 'Opening file ', filnam(4)
  open(20, file=filnam(4))
 end if
!Open the input file
 write(6,'(a,a)') 'Opening file ', filnam(1)
...
!Allocate dynamique variables
 allocate(phfrqqm1(3*natom))
 allocate(phfrq(3*natom))
 allocate(color(3,3*natom))
 allocate(qname(nlines+1))
 allocate(scale(nlines))
 allocate(nqptl(nlines))
 allocate(colorAtom(3,natom))
...
!Multiplication factor for units (from Hartree to cm-1 or THz)
 if(cunits==1) then
  facUnit=2.194746306726d5
 elseif(cunits==2) then
  facUnit=6.579683899978d3
 else
 end if
!calculate nqpt
 nqpt=0
...
&    *scale(jj)/renorm/(-nqpt)))
    ...
    if(filnam(4)/='no') then     !(in color)
     write(18,'(f5.3,a,f5.3,a,f5.3,a)') color(1,imode),' ', &
&     color(2,imode),' ',color(3,imode), ' srgb s gr'
    else
     write(18,'(f5.3,a,f5.3,a,f5.3,a)') 0.0,' ', &
&     0.0,' ',0.0, ' srgb s gr'
    end if
