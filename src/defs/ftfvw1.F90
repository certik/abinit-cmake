...
!End of the abilint section
    ...
    if(.not.ok) then
       ...
       allocate(lavnlfft(size(lavnlfft_in,1),size(lavnlfft_in,2)))
       allocate(vpion(size(vtrial_in,1),size(vtrial_in,2)))
       allocate(xccc3d(size(xccc3d_in)))
       allocate(kxc(1,0))
       allocate(rhog(2,nfftf),rhogsp(2,nfftf,nspden))
       allocate(nhat(nfftf,nspden*usepaw),nhatgr(nfftf,nspden,3*nhatgrdim))
       deltaW=deltaW_in
       gprimd=gprimd_in
       gsqcut=gsqcut_in
...
!!!!!!!!!!!!!!END OF VION COMPUTATION!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine ftfvw1__end()
    ...
    if(ok) then
       ...
       ok = .false.
       !! free memory
       deallocate(deltaW,lavnlfft,vpion,xccc3d,kxc,rhog,rhogsp)
       deallocate(nhat,nhatgr)
...
!End of the abilint section
    ...
    real(dp)            :: ftfvw1__e
    !Local variables-------------------------------
    integer ::option,cplex,ispden,ifftf
    ...
    if(ok) then
       ...
       call rhohxc(dtset,enxc,gsqcut,izero,kxc,mpi_enreg,nfftf,ngfft,&
            ...
            &  nkxc,nspden,n3xccc,option,rhog,rho,rprimd,dummy_strsxc,usexcnhat,vhartr,&
            &  vxc,vxcavg,xccc3d)
