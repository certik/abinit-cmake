...
type wvl_internalVars_type
  ! Size of the coarse grid (without buffer values).
  ! The full data arrays are (2 * %nSize(1) + %buffer) * (2 * %nSize(2)...
  ! These values can be extracted from dtset%ngfft :
  ! nSize(i) = (dtset%ngfft - %buffer) / 2
  integer :: nSize(3)
  ! Size of the buffer (i.e. expansion of the wavelet basis).
  integer :: buffer

  ! Size of the fine grid used to expand potentials, densities...
  ! Each dimensions are equal to 2 * nSize(i) + buffer.
  integer :: dpSize(3)
  ! Number of elements in the density/potential grid,
  ! equals to dpSize(1) * dpSize(2) * dpSize(3)
  integer :: nDpPoints

  ! Localisation information of a cube containing the fine grid.
  ! (1,:) gives the lower point and (2,:) the higher one.
  integer :: fineGrid(2, 3)
...
! Integer
  ...
  integer :: phfrqexist,phfrqwrite,piezoflag,polflag,prtdos,prtmbm,prtfsurf,prtnest,ramansr
...
!!****t* defs_datatypes/coulombian_type
!! NAME
!!  coulombian_type
!!
!! FUNCTION
!!  This data type contains the square root of the Fourier components of the coulombian interaction 
!!  calculated taking into account a possible cutoff. Info on the particular geometry used for the cutoff 
!!  as well as quantities required to deal with the coulombian divergence are reported as well.
!!
!! SOURCE

 type Coulombian_type

  integer  :: nfft                         ! Number of points in FFT grid
  integer  :: ng                           ! Number of G-vectors
  integer  :: nqibz                        ! Number of irreducible q-points
  integer  :: nqsmall                      ! Number of small q-points around Gamma

  real(dp) :: alpha(3)                     ! Lenght of the finite surface
  real(dp) :: rcut                         ! Cutoff radius
  real(dp) :: i_sz                         ! Value of the integration of the Coulombian singularity 4\pi/V_BZ \int_BZ d^3q 1/q^2
  ...
  integer(i1b),pointer :: ctab(:)          ! For each point of the FFT grid this table reports
  ...
  complex(gwpc),pointer :: vc_sqrt(:,:)    ! Square root of the coulombian in reciprocal space
  complex(gwpc),pointer :: vcqs_sqrt(:,:)  ! Square root of the coulombian calculated for small q-points

 end type Coulombian_type
...
&  getwfq,getxcart,getxred,get1den,get1wf,gwcalctyp,gwcomp,gwgamma,gwmem,gwpara,iboxcut,&
&  icoulomb,icutcoul,idyson,ieig2rf,ikhxc,inclvkb,intexact,intxc,ionmov,&
&  iprcch,iprcel,iprctfvw,iprcfc,irdddk,irdkss,irdqps,irdscr,irdsuscep,irdwfk,irdwfq,ird1wf,&
&  iscf,isecur,istatr,istatshft,ixc,ixcpositron,&
!  jdtset contains the actual number of the dataset
&  jdtset,jellslab,kpara,kptopt,kssform,ldgapp,localrdwf,lofwrite,mband,mffmem,mgfft,mgfftdg,&
&  mkmem,mkqmem,mk1mem,nnos,&
&  mpw,mqgrid,mqgriddg,natom,natpawu,natrd,natsph,natvshift,nbandkss,nbandsus,nbdblock,nbdbuf,&
&  nberry,ncenter,nconeq,nctime,ndtset,ndyson,&
&  nfft,nfftdg,nfreqim,nfreqmidm,nfreqre,nfreqsp,nfreqsus,ngeohist,ngroup_rf,nkptgw,nkpt,nline,&
&  nnsclo,nomegasf,nomegasi,nomegasrd,norb,npack,npara,npband,npfft,npkpt,npsp,npspalch,npulayit,&
...
&  outputXML,paral_kgb,paral_rf,parareel,&
&  pawlcutd,pawlmix,pawmixdg,pawnhatxc,pawnphi,pawntheta,pawnzlm,pawoptmix,&
&  pawprtdos,pawprtvol,pawspnorb,pawstgylm,pawusecp,pawxcdev,&
&  positron,ppmodel,prepanl,prepgkk,prtacfd,prtbbb,prtcml,prtcs,&
&  prtden,prtdensph,prtdos,prtdosm,prteig,prtefg,prtfc,prtfsurf,prtgeo,prtgkk,prtkpt,&
&  prtnabla,prtpmp,prtpot,prtspcur,prtstm,prtvha,prtvhxc,prtvol,prtvxc,&
&  prtwant,prtwf,prt1dm,ptgroupma,rdmnb,recnpath,recnrec,recptrott,rectesteg,restartxf,rfasr,rfelfd,&
&  rfmeth,rfmgfd,rfphon,rfstrs,rfthrd,&
&  rfuser,rf1elfd,rf1phon,rf2elfd,rf2phon,rf3elfd,rf3phon,&
&  signperm,spgaxor,spgorig,spgroup,splitsigc,spmeth,suskxcrs,symmorphi,symchi,symsigma,&
&  td_mexcit,tfkinfunc,timopt,tl_nprccg,usedmatpu,useexexch,usepaw,&
&  usepawu,useria,userib,useric,userid,userie,usewvl,useylm,vacnum,wfoptalg,wvl_nprccg,w90iniprj,w90prtunk,w90nplot,xclevel
! Integer arrays
  integer :: bdberry(4),dsifkpt(3),kptrlatt(3,3),ngfft(18),ngfftdg(18),nloalg(5),&
&  qprtrb(3),rfatpol(2),rfdir(3),rf1atpol(2),rf1dir(3),&
&  rf2atpol(2),rf2dir(3),rf3atpol(2),rf3dir(3),supercell(3),w90cplot(3)
! Integer pointers
...
&  omegasimax,omegasrdmax,pawecutdg,pawovlp,pawsphmix,ppmfrq,qptnrm,recrcut,recefermi,rectolden,rhoqpmix,rcut,&
&  sciss,slabwsrad,slabzbeg,slabzend,soenergy,spbroad,stmbias,strfact,strprecon,&
...
&  zcut
! Types
  type(wvl_internalVars_type) :: wvl_internal
! Real arrays
...
!scalars
  ...
  logical :: rpa                    ! if true use RPA expression for the screening
  logical :: tddft                  ! if true include vertex correction from TDDFT
  logical :: static                 ! if true only omega=0 is calculated
  logical :: testelectron           ! if true use testelectron screening 
  logical :: testparticle           ! if true use testparticle screening

!arrays
  integer :: mG0(3)                 ! For each reduced direction gives the max G0 component to account for umklapp processes

  real(dp),pointer :: qlwl(:,:)     ! nqlwn q-points for the long-wavelength limit TODO should be removed
  real(dp),pointer :: omegasf(:)    ! real frequencies used to calculate the spectral function

  complex(gwpc),pointer :: omega(:)       ! frequencies for chi0,epsilon and epsilonm1
...
!arrays
  integer :: ngfft(18)                         ! All needed information about 3D FFT, 
                                               ! see ~abinit/doc/input_variables/vargs.htm#ngfft
  integer,pointer :: igfft0(:)                 ! Index of G vectors in the FFT array

  logical,pointer :: is_already_stored(:,:,:)  ! If .TRUE. \psi(r) is in memory

  complex(gwpc),pointer :: wfg(:,:,:,:)        ! Wavefunctions in real space 
  complex(gwpc),pointer :: wfr(:,:,:,:)        ! Wavefunctions in reciprocal space 
...
#include "config.h"
 ...
 type Gpairs_type
  ...
  integer :: ngpairs                 ! Number of G pairs ie ng**2, used to dimension gptab and gptabo
...
#include "config.h"
 ...
 type Gvectors_type
  ...
  integer,pointer :: g2sh(:)                 ! For each G gives the shell to which it belongs
  integer,pointer :: gvec(:,:)               ! Reduced coordinates of G vectors 
  integer,pointer :: rottb(:,:,:)            ! grottb(G,I,S) is the index of (SI) G in the array gvec
  integer,pointer :: rottbm1(:,:,:)          ! grottbm1(G,I,S) is the index of IS{^-1} G in the array gvec
  integer,pointer :: shlim(:)                ! shlim(nsh+1) Index of the first G vector in each shell, ng+1 for nsh+1

  real(dp) :: gmet(3,3)                      ! Reciprocal space metric ($\textrm{bohr}^{-2}$).
  real(dp) :: gprimd(3,3)                    ! Dimensional reciprocal space primitive translations (b^-1)

  real(dp),pointer :: shlen(:)               ! shlen(nsh+1)=radius of each shell.

  complex(gwpc),pointer :: phmGt(:,:)        ! phmGt(ng,nsym) contains the phase factor e^{-i2\pi(G.\tau)} where
                                             ! $\tau$ is the fractional translation associated to isym

  complex(gwpc),pointer :: phmSGt(:,:)       ! phmSGt(ng,nsym) contains the phase factor e^{-i2\pi(SG.\tau)} where
                                             ! where S is one of the symmetry properties in reciprocal space
...
!scalars
 integer :: nbz,nibz               ! Number of points in the BZ, IBZ, respectively 
 integer :: nsmall                 ! Number of small points around Gamma
 integer :: nsym                   ! Number of symmetry operations
 integer :: timrev                 ! 2 if time reversal symmetry can be used, 1 otherwise
!arrays
 integer,pointer :: rottb(:,:,:)   ! rottb(k,I,S), index of (IS)k in the array BZ. S is one the nsym operations in 
                                   ! reciprocal space, I is the identity or the inversion operator (1,2 resp) 
 integer,pointer :: rottbm1(:,:,:) ! rottbm1(k,I,S), index no. of IS^{-1} k
 integer,pointer :: tab(:)         ! For each point in the BZ, the index of the irreducible point in the array ibz
 integer,pointer :: tabi(:)        ! For each point in the BZ, tabi tells if time-reversal has to be used to obtain k_BZ 
                                   ! starting from the correponding point k_IBZ  (1=>no, -1=>yes)
 integer,pointer :: tabo(:)        ! For each point in the BZ, the index, in the array symrec, of the symmetry operation in 
                                   ! reciprocal space which rotates k_IBZ ! onto \pm k_BZ (depending on tabi)
 real(dp) :: gmet(3,3)             ! reciprocal space metric ($\textrm{bohr}^{-2}$).
 real(dp) :: gprimd(3,3)           ! dimensional primitive translations for reciprocal space ($\textrm{bohr}^{-1}$)
 
 real(dp),pointer :: bz(:,:)       ! Reduced coordinates of the points in the BZ
 real(dp),pointer :: ibz(:,:)      ! Reduced coordinates of the points in the IBZ
 real(dp),pointer :: small(:,:)    ! Reduced coordinates of the small points around Gamma
 real(dp),pointer :: wt(:)         ! Weights for each point in the IBZ

 complex(gwpc),pointer :: tabp(:)  ! For each point in the BZ, the phase factors associated to non-symmorphic operations, 
                                   ! i.e e^{-i 2\pi k_IBZ.R{^-1}t}==e{-i 2\pi k_BZ cdot t}
...
!Integer scalars
  ...
  integer :: has_dijhartree
   ! 1 if paw_ij%dijhartree is allocated

  integer :: has_dijhat
   ! 1 if paw_ij%dijhat is allocated
  ...
  integer :: ndij
   ...
   ! Usually ndij=nspden, except for spin-orbit (where ndij=npsinor**4)
...
!Real (real(dp)) arrays
  ...
  real(dp), pointer :: dijhat(:,:)
   ...
   ! Dij_hat term (non-local operator)
   ! Same storage as Dij (see above)
...
!Real (real(dp)) arrays
  ...
  real(dp), pointer :: grhoij (:,:,:)
   ! grhoij(cplex*ngrhoij,lmn2_size,nspden)
...
#include "config.h"
 ...
 type sigma_parameters
  ...
  integer :: npwwfn                      ! Number of planewaves for wavefunctions
  integer :: npwx                        ! Number of planewaves for $\Sigma_x$
  integer :: npwc                        ! Number of planewaves for $\Sigma_c$ and W
  integer :: nbnds                       ! Number of bands kept in the calculation
  integer :: nomegasr                    ! Number of frequencies on the real axis where evaluate the spectral function
  integer :: nomegasrd                   ! Number of frequencies on the real axis where evaluate $\Sigma(E)$
  integer :: nomegasi                    ! Number of frequencies along the imaginary axis used for Sigma in case 
                                         ! of analytic continuation  
  integer :: nsig_ab                     ! Number of components in the self-energy operator (1 if nspinor==1, 4 if nspinor==2)
  integer :: nspinor                     ! Number of spinorial components.
  integer :: nsppol                      ! 1 for unpolarized, 2 for spin-polarized calculation
  integer :: nkcalc                      ! Number of k-points where GW corrections have been calculated
  integer :: ppmodel                     ! Integer defining the plasmon pole model used (if used)
  ...
  logical :: oneshot,onlyenergy,full     !PRESENTLY NOT USED TODO
  logical :: GW,modelGW,HF,COHSEX,SEX
  logical :: PPM,AC,CD
  ...
  real(dp) :: omegasrmax                 ! Maximum real frequency for the evaluation of the spectral function
  real(dp) :: omegasrdmax                ! Maximum displacement around the KS energy where evaluate the diagonal
                                         ! Elements of $ \Sigma(E)$
  ...
  integer,pointer :: kcalc(:)            ! Gives, for each k-point where GW corrections are calculated,
                                         ! the corresponding index in the BZ
  integer,pointer :: minbnd(:),maxbnd(:) ! For each k-point define the min and max band index where evaluate the GW corrections
  real(dp),pointer :: xkcalc(:,:)        ! Reduced coordinates of the k-points where GW corrections have been calculated

  complex(dpc),pointer :: omegasi(:)     ! Frequencies along the imaginary axis used for the analytical continuation
  complex(dpc),pointer :: omegasf(:)    ! Frequencies used to evaluate the spectral function
...
#include "config.h"
 ...
 type sigma_results
  ...
  integer :: nomega                            ! No. of real frequencies for the spectral function.
  integer :: nomegasrd                         ! No. of real frequencies to evaluate $\Re\Sigma(E)$.

  real(dp),pointer :: ak(:,:)                  ! FIXME (to be described)
  real(dp),pointer :: ame(:,:,:)               ! FIXME (to be described)
  real(dp),pointer :: degwgap(:,:)             ! Difference btw the QP and the KS gap, for each k-point and spin
  real(dp),pointer :: egwgap(:,:)              ! QP gap for each k-point and spin
  real(dp),pointer :: en_qp_diago(:,:,:)       ! QP energies obtained from the diagonalization of Sigma
  real(dp),pointer :: e0(:,:,:)                ! KS eigenvalues for each band, k-point and spin. In case of self-consistent? 
  real(dp),pointer :: e0gap(:,:)               ! KS gap at each k-point considered in the calculation
  real(dp),pointer :: sigxme(:,:,:)            ! Diagonal elements of $\Sigma_x$ : $\langle nks | \Sigma_x | nks \rangle$
  real(dp),pointer :: vxcme(:,:,:)             ! $\langle nks|v_{xc}[nval]|nks\rangle$ valence-only matrix elements
  real(dp),pointer :: vUme(:,:,:)              ! $\langle nks|v_{U}|nks\rangle$ for LDA+U

  complex(dpc),pointer :: degw(:,:,:)         ! Difference between the QP and the KS energies,
  complex(dpc),pointer :: dsigmee0(:,:,:)     ! Derivative of $\Sigma_c(E)$ calculated at the KS eigenvalue.
  complex(dpc),pointer :: egw(:,:,:)          ! QP energies, $\epsilon_{nks}^{QP}$
  complex(dpc),pointer :: eigvec_qp(:,:,:,:)  ! Expansion of the QP amplitude in the KS basis set
  complex(dpc),pointer :: hhartree(:,:,:,:)   ! $\<nks| T + v_{loc} + v_{nl} + v_{H} |mks\>$
  complex(dpc),pointer :: sigcme(:,:,:,:)     ! $\<nks| \Sigma_{c} (E) |nks\>$ at each Sr%nomega frequency
  complex(dpc),pointer :: sigmee(:,:,:)       ! $\Sigma_{xc} E_{KS} + (E_{QP}- E_{KS}) 
  complex(dpc),pointer :: sigcmee0(:,:,:)     ! Diagonal elements of $\Sigma_c (E)$ calculated at the KS energy $E_{KS}$
  complex(dpc),pointer :: sigcmesi(:,:,:,:)   ! Matrix elements of $\Sigma_c$ along the imaginary axis
  complex(dpc),pointer :: sigcmesrd(:,:,:,:)  ! diagonal matrix elements of \Sigma_c for several frequencies around
  complex(dpc),pointer :: sigxcme(:,:,:,:)    ! $\<nks| \Sigma_{xc} (E) |nks\>$ at each sr%nomega frequency
  complex(dpc),pointer :: sigxcmesi(:,:,:,:)  ! Matrix elements of $\Sigma_{xc}$ along the imaginary axis
  complex(dpc),pointer :: sigxcmesrd(:,:,:,:) ! Diagonal matrix elements of \Sigma_xc at frequencies around the KS eigenvalue
  complex(dpc),pointer :: ze0(:,:,:)          ! $(1 -\frac {\partial \Sigma_c} {\partial E_{KS}})^{-1} $

  complex(dpc),pointer :: omegasrd(:,:,:,:)    ! sr%omegasrd(jb,jkibz,io,is) contains the frequencies that that will be used
...
#include "config.h"
 ...
 type little_group
  ...
  integer :: npw                       ! Number of planewaves used to describe the wavefuntion, used to dimension igmG0
  integer :: nsym_sg                   ! Number of operations in the space group (*NOT* the little group)
  integer :: nsym_ltg                  ! Number of symmetry operations in the little group (time-reversal is included, if can be used)
  integer :: timrev                    ! 2 if time-reversal is considered, 1 otherwise
  integer :: nbz                       ! Number of kpoints in the full BZ
  integer :: nibz_ltg                  ! Number of point in the irreducible wedge defined by the little group
  !integer :: use_umklp                 ! 1 if umklapp processes are included 

  real(dp) :: max_kin_gmG0

  integer,pointer :: G0(:,:,:)         ! g0(3,timrev,nsym_sg), reduced coordinates of the umklapp G0 vector
  integer,pointer :: ibzq(:)           ! ibzq(nbz): 1 if the point belongs to the IBZ_q defined by ext_pt, 0 otherwise
  integer,pointer :: bz2ibz(:)         ! bz2ibz(nbz) gives the sequential index of the point in the irreducible wedge 
                                       ! defined by the little group, 0 otherwise
  integer,pointer :: ibz2bz(:)         ! For each nibz_ltg the correspondind index in the BZ array
  integer,pointer :: igmG0(:,:,:)      ! iumklp(G,I,S) gives the index of G-G0 in the FFT array for each operations IS (I=\pm 1)
  integer,pointer :: flag_umklp(:,:)   ! flag_umklp(I,S) is 1 if the operation IS requires a non null G0 vector to preserve q, 0 otherwise
  integer,pointer :: preserve(:,:)     ! preserve(1,S) is 1 if the operation S in rec space preserves the external q-point i.e Sq=q+G0 
                                       ! preserve(2,S) is 1 if -Sq=q+G0. G0 is a reciprocal lattice vector also called "umklapp vector"
  integer,pointer :: tab(:)            ! tab(nbz) gives, for each point in BZ, the index of the irreducible point (kIBZ_q) in the irreducible 
                                       ! wedge defined by the little group of q. kBZ= (IS) kIBZ where I is the inversion or the identity
  integer,pointer :: tabo(:)           ! tabo(nbz)=the index of the operation S in the little group that rotates ! kIBZ_q into \pm kBZ
  integer,pointer :: tabi(:)           ! tabi(nbz)=for each k-point in the BZ defines whether inversion has to be
                                       ! considered in the relation kBZ= IS kIBZ_q (1 => only S; -1 => -S)
  integer,pointer :: wtksym(:,:,:)     ! wtksym(I,S,kbz): 1 if IS belongs to the little group, 0 otherwise !(should invert firt dimensions

  real(dp)         :: ext_pt(3)        ! The external point defining the little group
...
type Bands_Symmetries
 ...
 integer :: nspinor                      ! Spinorial case
 integer :: nsppol                       ! Number of independent spin polarizations
 integer :: nbnds                        ! Number of bands for this k-point (same for up and down spin)
 integer :: timrev                       ! 2 if time-reversal is used, 1 otherwise
 integer :: nclass                       ! The number of classes in the small group 
 integer :: nsym_sgk                     ! Number of symmetries in the small/little group of k
                                         ! one operation has a non-zero fractional translation
 ...
 real(dp) :: kpt(3)                      ! The k-point under investigation

 integer,pointer :: G0(:,:)              ! The umklapp G0 vector associated to each little group operation
 integer,pointer :: ncplx(:)             ! Number of degenerate states, for each spin
 integer,pointer :: nelements(:)         ! nelements(1:nclass) Number of operations in each class
 integer,pointer :: sgk2symrec(:)        ! sgk2symrec(nsym_sgk) the index of each symmetry in the symrec array
                                         ! indeces are packed by classes 
...
type Degenerate_Bands
 ...
 integer :: dim_cplx                     ! Number of degenerate bands (dim of representation if no accidental degeneracy occurs)
 integer :: ib_start,ib_end              ! Starting and final band index for the set of degenerate states
 real(dp),pointer :: ene(:)              ! The energies (supposed to be almost generate)
 complex(dpc),pointer :: Rirr(:,:,:)    ! Rirr(dim_cplx,dim_cplx,nsym_sgk) the irreducible representations
 complex(dpc),pointer :: trace(:)       ! trace(nclass) trace of the irreducible representations
...
!scalars
  !integer :: sp_group,pt_group              ! Space and point group 
...
!arrays
  real(dp) :: angdeg(3)                      ! Angles among rprim (degree)
  real(dp) :: rmet(3,3)                      ! Metric in real space
  real(dp) :: rprimd(3,3)                    ! Direct lattice vectors, Bohr units

  integer,pointer :: indsym(:,:,:)           ! indsym(4,nsym,natom)=indirect indexing array for atoms, see symafm.F90
  integer,pointer :: symafm(:)               ! Ferromagnetic symmetries
  integer,pointer :: symrec(:,:,:)           ! Symmetry operation in reciprocal space (reduced coordinates)
  integer,pointer :: symrel(:,:,:)           ! Symmetry operations in direct space (reduced coordinates)
  integer,pointer :: atindx(:),atindx1(:)    ! Index tables for atoms
  integer,pointer :: typat(:),nattyp(:)      ! Type for each natom and number of atoms of each type

  real(dp),pointer :: tnons(:,:)             ! Fractional translations
  real(dp),pointer :: xcart(:,:),xred(:,:)   ! Cartesian and Reduced coordinates
  real(dp),pointer :: spinrot(:,:)           ! spinrot(4,nsym)= spinor rotation matrices.
...
!arrays
  complex(gwpc),pointer :: bigomegatwsq(:,:,:)  ! Plasmon pole parameters $\tilde \Omega^2_{G Gp} (q) $
  complex(gwpc),pointer :: omegatw(:,:,:)       ! Plasmon pole parameters $\tilde \omega_{G Gp} (q) $     end type
  complex(gwpc),pointer :: eigpot(:,:,:)        ! Eigvectors of the symmetrized inverse dielectric matrix
...
#include "config.h"
 ...
 type Epsilonm1_results
  ...
  integer :: ID                          ! Response function identifier: 0 for e^-1, 1 for chi0.
  integer :: ikxc                        ! Kxc kernel used, 0 for None (RPA), >0 for static TDDFT (=ixc), <0 for TDDFT 
  integer :: inclvkb                     ! q-->0 treatment, 0 for None, 1-2 for transversal gauge, <0 for longitudinal
  ...
  integer :: nqibz                       ! Number of q-points in the IBZ.
  integer :: nqlwl                       ! Number of point for the treatment of the long wave-length limit.
  integer :: nomega                      ! Number of frequencies where $\tilde\epsilon(\omega)$ has been evaluated.
  integer :: nomega_file                 ! Number of frequencies reported in the external file.
  integer :: nomega_i                    ! Number of purely imaginary frequencies.
  integer :: nomega_r                    ! Number of real frequencies.
  integer :: nbnds_used                  ! Number of bands used during the screening calculation (info only)
  integer :: npwe                        ! Number of G vectors used.
  integer :: npwe_file                   ! Number of G vectors reported on file.
  integer :: npwwfn_used                 ! Number of plane waves used during the screening calculation (info only)
  integer :: test_type                   ! 0 for TEST-PARTICLE, 1 for TEST-ELECTRON (only for TDDFT)
  integer :: Tordering                   ! 0 for Time-Ordered, 1 for Advanced, 2 for Retarded.
  
  character(len=fnlen) :: fname          ! Name of file from which epsm1 is read.

!arrays
  integer,pointer  :: gvec(:,:)                 
  ! gvec(3,Er%npwe_file) 
  ! G vectors used in reciprocal lattice units.
  ...
  ! qlwl(3,Er%nqlwl)
  ! q-points for the long wave-length limit treatment.

  complex(gwpc),pointer :: epsm1(:,:,:,:)       
  ! epsm1(Er%npwe,Er%npwe,Er%nomega,Er%nqibz)
  ! Contains the two-point function $\epsilon_{G,Gp}(q,omega)$ in frequency and reciprocal space.

  complex(gwpc),pointer :: lwing(:,:,:)         
  ! lwing(Er%npwe,Er%nomega,Er%nomega,Er%nqlwl)
  ! Only if chi0, lower wing for the different q"s -->0 

  complex(gwpc),pointer :: omega(:)             
  ! omega(Er%nomega_file) TODO recheck dimensions
  ! Frequencies used both along the real and the imaginary axis.

  complex(gwpc),pointer :: uwing(:,:,:)   
  ! uwing(Er%npwe,Er%nomega,Er%nomega,Er%nqlwl)
  ! Only if chi0, lower wing for the different q"s -->0 

  type(Hdr_type) :: Hdr                         
  ! The header reported in the _SCR of _SUSC file. 

  ! TODO use this object once implementation is completed.
  !type(File_handler) :: Fh                     
  ! Structure containing information on the external file.
