...
!!
!! FUNCTION
!! Conducts printing inside the scfcv.f routine,
!! according to the value of choice.
!! Also checks the convergence with respect to the different
!! criteria.
...
!!   | outputXML=1 if values have to be stored in an XML file.
!!   | prteig=
...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 ...
 use interfaces_15common, except_this_one => scprqt
...
!scalars
 ...
 real(dp) :: magnet,rhodn,rhoup
 logical :: noquit
 character(len=500) :: message
 character(len=8) :: colname
!arrays
 real(dp) :: f_tmp(3),tsec(2)

! *********************************************************************

!DEBUG
!write(6,*)' scprqt : enter, iscf,nstep= ',iscf,nstep
!stop
!ENDDEBUG
...
! Examine tolerance criteria
  ...
  tolvrs=tollist(6)
  tolrff=tollist(7)
  ttolwfr=0 ; ttoldff=0 ; ttoldfe=0 ; ttolvrs=0
...
! Print eigenvalues every step if dtset%prtvol>=10 and GS case
  if (dtset%prtvol>=10 .and. response==0 .and. dtset%tfkinfunc==0) then
   option=1
   call prteigrs(eigen,dtset%enunit,fermie,filapp,ab_out,iscf,kpt,dtset%kptopt,dtset%mband,&
&   nband,nkpt,dtset%nnsclo,dtset%nsppol,occ,dtset%occopt,option,dtset%prteig,dtset%prtvol,resid,tolwfr,vxcavg,wtk)
   call prteigrs(eigen,dtset%enunit,fermie,filapp,  06,iscf,kpt,dtset%kptopt,dtset%mband,&
&   nband,nkpt,dtset%nnsclo,dtset%nsppol,occ,dtset%occopt,option,dtset%prteig,dtset%prtvol,resid,tolwfr,vxcavg,wtk)
  end if
...
! In a special case, do not quit even if convergence is reached
...
!  input tolrff times fcart itself twice consecutively, stop steps (exit loop here)
   if( ttolrff==1 ) then
    if( istep==1 )then
     tolrff_ok=0
    else if (diffor<tolrff*maxfor) then
...
!  is less than input tolvrs, stop steps (exit loop here)
   if( ttolvrs==1 .and. res2<tolvrs) then
    if (optres==0) then
