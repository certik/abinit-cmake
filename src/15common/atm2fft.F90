...
!!
!! PARENTS
!!      dyfro3,forces,prcref,respfn,setvtr,stress
!!
!! CHILDREN
...
!Zero out arrays to permit accumulation over atom types
 if (optv==1.and.optatm==1) then
  allocate(workv(2,nfft));workv(:,:)=zero
 end if
 if (optn==1.and.optatm==1) then
  allocate(workn(2,nfft));workn(:,:)=zero
 end if
 if (optv==1.and.optgr==1) then
  allocate(grv_indx(3,natom));grv_indx(:,:)=zero
 end if
 if (optn==1.and.optgr==1) then
  allocate(grn_indx(3,natom));grn_indx(:,:)=zero
 end if
 if (optv==1.and.optdyfr==1) then
  allocate(dyfrv_indx(3,3,natom));dyfrv_indx(:,:,:)=zero
 end if
 if (optn==1.and.optdyfr==1) then
  allocate(dyfrn_indx(3,3,natom));dyfrn_indx(:,:,:)=zero
 end if
...
!
 ...
 allocate(phre_igia(natom),phim_igia(natom))
...
! End loop on type of atoms
 ...
 deallocate(phre_igia,phim_igia)
...
& (optgr==1.or.optstr==1.or.optdyfr==1)) then
  ...
  if (optv==1) then
   if (optgr==1)   call xsum_mpi(grv_indx,spaceComm,ierr)
   if (optstr==1)  call xsum_mpi(strv,spaceComm,ierr)
   if (optdyfr==1) call xsum_mpi(dyfrv_indx,spaceComm,ierr)
  end if
  if (optn==1) then
   if (optgr==1)   call xsum_mpi(grn_indx,spaceComm,ierr)
   if (optstr==1)  call xsum_mpi(strn,spaceComm,ierr)
   if (optdyfr==1) call xsum_mpi(dyfrn_indx,spaceComm,ierr)
  end if
...
!Dynamical matrix: re-order atoms
 if (optdyfr==1) then
  if (optv==1) then
   ...
   deallocate(dyfrv_indx)
  end if
  if (optn==1) then
   do ia=1,natom
    dyfrn(1:3,1:3,atindx1(ia))=dyfrn_indx(1:3,1:3,ia)
   end do
   deallocate(dyfrn_indx)
  end if
 end if

!DEBUG
!write(6,*)' atm2fft : exit '
!ENDDEBUG
