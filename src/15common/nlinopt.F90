...
!!
!! COPYRIGHT
!! Copyright (C) 2002-2008 ABINIT group (SSharma)
...
!! SIDE EFFECTS
!!  pmat(nstval,nstval,nkpt,3,nspin) = momentum matrix elements in cartesian coordinates(complex)
!!                                     : changes on exit, due to scissor correction
!!
!! COMMENTS
...
integer :: isp,isym,ik
integer :: ist,ist1,ist2,istl,istn,istm
real(dp) :: ha2ev
real(dp) :: t1,t2,t3,t4,tst
real(dp) :: ene,totre,totabs,totim
real(dp) :: corec,e1,e2,e12,e21,el,en,em
real(dp) :: emin,emax
real(dp) :: const,const_au,au2esu
...
!  test if off-diagonal elements are zero
   ...
   abs(symcrys(3,1,isym)).lt.tst.and.abs(symcrys(3,2,isym)).lt.tst) then
    print *,'-----------------------------------------'
    print *,'    the crystal has inversion symmetry   '
    print *,'    the SHG sussceptibility is zero      '
    print *,'-----------------------------------------'
    stop
   end if
  end if
...
!check polarisation
 if (v1.le.0.or.v2.le.0.or.v3.le.0.or.v1.gt.3.or.v2.gt.3.or.v3.gt.3) then
  print *,'---------------------------------------------'
  print *,'    Error in nlinopt:                        '
  print *,'    the polarisation directions incorrect    '
  print *,'    1=x,  2=y  and 3=z                       '
  print *,'---------------------------------------------'
  stop
 end if
...
!tolerance
 if (tol.gt.0.006) then
  print *,'----------------------------------------'
  print *,'    ATTENTION: tolerance is too high    '
  print *,'    ideally should be less than 0.004   '
  print *,'----------------------------------------'
 end if
...
!allocate local arrays
 allocate(px(nstval,nstval,3,3,3))
 allocate(py(nstval,nstval,3,3,3))
 allocate(pz(nstval,nstval,3,3,3))
 allocate(inter2w(nmesh))
 allocate(inter1w(nmesh))
 allocate(intra2w(nmesh))
 allocate(intra1w(nmesh))
 allocate(delta(nstval,nstval,3))
 allocate(sym(3,3,3))
 allocate(s(3,3))
    ...
    if (e1.lt.efermi) then
     do ist2=1,nstval
      ...
      if (e2.gt.efermi) then
       e12=e1-e2
!      correct the momentum matrix elements for scissors shift
       corec=(e12-sc)/e12
       pmat(ist1,ist2,ik,1:3,isp)=pmat(ist1,ist2,ik,1:3,isp)*corec
...
!  start the calculation
   do istn=1,nstval
    ...
    if (en.lt.efermi) then
     do istm=1,nstval
      em=evalv(istm,isp,ik)
      if (em.gt.efermi) then
       em=em+sc
       wmn=em-en
       wnm=-wmn
!      calculate the matrix elements for two band intraband term
       ...
       do lx=1,3
        do ly=1,3
          ...
          *delta(istm,istn,lz)
!         NOTE:: lx to ly m to n in pmat-s respectively (AG: changed apostrophe to -)
...
!      
       b11=b11*zi*(1._dp/wnm)*const
       b12_13=b12_13*zi*(1._dp/wnm)*const
       b24=(b24+b231)*zi*(1._dp/(wnm**3))*const
       b21_22=(b21_22)*zi*(1._dp/(wnm**3))*const
       b31_32=(b31_32-b331)*zi*(1._dp/(wmn**3))*const*0.5_dp
...
!deallocate local arrays
 deallocate(px,py,pz,sym,s)
 deallocate(inter2w,inter1w,intra2w,intra1w,delta)
 close(92)
 close(93)
 close(94)
 close(95)
...
!print information
 print *, ' '
 print *, 'information about calculation just performed:'
 print *, ' '
 print *, 'calculated the component:',v1,v2,v3 ,'of second order susceptibility'
 print *, 'tolerence:',tol
 if (tol.gt.0.008) print *,'ATTENTION: tolerence is too high'
 print *, 'broadening:',brod,'Hartree'
 if (brod.gt.0.009) then
  print *, ' '
  print *,'ATTENTION: broadening is quite high'
  print *, ' '
 ...
 print *, 'scissors shift:',sc,'Hartree'
 print *, 'energy window:',(emax-emin)*ha2ev,'eV',(emax-emin),'Hartree'
!
 return
