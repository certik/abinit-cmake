...
!Local variables-------------------------------
!scalars
 integer :: iatom,idir,ifft,iout
!arrays
 real(dp) :: dedv_temp(1)
 real(dp),allocatable :: ddens(:,:,:),vresid(:,:)
...
!Dot product ddens(:,:,1) f_fftgr(:,:,i_vresid(2))
 call dotprodm_vn(cplex,1,ddens,dedv_temp,1,i_vresid(2),mpi_enreg,1,1,1,&
& nfft,nfftot,n_fftgr,nspden,f_fftgr,ucvol)
 dedv_old = dedv_temp(1)

!Dot product ddens(:,:,1) f_fftgr(:,:,i_vresid(1))
 call dotprodm_vn(cplex,1,ddens,dedv_temp,1,i_vresid(1),mpi_enreg,1,1,1,&
& nfft,nfftot,n_fftgr,nspden,f_fftgr,ucvol)
 dedv_new= dedv_temp(1)

 if(choice==3)then
! Dot product ddens(:,:,1) f_fftgr(:,:,i_vresid(3))
  call dotprodm_vn(cplex,1,ddens,dedv_temp,1,i_vresid(3),mpi_enreg,1,1,1,&
&  nfft,nfftot,n_fftgr,nspden,f_fftgr,ucvol)
