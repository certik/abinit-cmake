...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 ...
 use interfaces_12parser, except_this_one => intagm
...
! (1) try to find the token with dataset number appended
  if(jdtset>0)then
   ...
   if(jdtset<10)cslen=toklen+3
   if(jdtset>=10)cslen=toklen+4
...
&   '  Action : check your input file.'
  ...
  typevar='KEY'
! DEBUG
! write(6,*)' intagm : will read cs=',trim(cs)
! stop
! ENDDEBUG
 end if
