...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2008 ABINIT group (GMR)
...
subroutine symq3(nsym,qpt,symq,symrec,timrev)
...
!Local variables -------------------------
!scalars
 integer :: ii,isign,isym,itirev,jj
...
!  Get the difference between the symmetric
!  and the original vector

   symq(4,itirev,isym)=1
   do ii=1,3
    difq(ii)=qsym(ii)-qpt(ii)
!   Project modulo 1 in the interval ]-1/2,1/2]
!   such that difq = reduce + shift
    call canon9(difq(ii),reduce,shift(ii))
    if(abs(reduce)>tol)symq(4,itirev,isym)=0
   end do

!  If the operation succeded, change shift from
!  real(dp) to integer, then exit loop
   if(symq(4,itirev,isym)/=0)then
    if(itirev==1)write(message, '(a,i4,a)' )&
&    ' symq3 : found symmetry',isym,' preserves q '
    if(itirev==2)write(message, '(a,i4,a)' )&
&    ' symq3 : found symmetry ',isym,' + TimeReversal preserves q '
...
!  End do-loop on symmetries and time-reversal
  end do
 end do
...
! call canon9(2*qpt(ii),reduce,shift(ii))
