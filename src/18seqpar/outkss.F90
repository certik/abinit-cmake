...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2008 ABINIT group (MT, VO, AR, MG)
...
!!
!! TODO
!!  Collect cprj on master node in case of PAW+ parallel execution.
!!
!! TESTS
...
!scalars
 ...
 integer :: spinor_shift1,spinor_shift2
 ...
 integer :: k_index,kk,klmn,lmn2_size,matblk,maxpw,mcg_disk,mproj,n1,n2,n3,n4,n5,n6,nband_k,nb
...
&   '  Program does not stop but _KSS file will not be created...'
  ...
  if (usecprj/=1) then
   write(msg,'(4a)')ch10,&
&   ' For PAW and kssform=1 usecpry must be 1',ch10,&
&   '  Program does not stop but _KSS file will not be created...'
   call wrtout(std_out,msg,'COLL') 
   call wrtout(ab_out,msg,'COLL') ; RETURN
  end if
...
#endif
 !
 ! Estimate required memory TODO to be modified to take into account the case nsppol=2
 if (kssform/=3) then 
...
&   ' the program will perform complete diagonalizations.'
 ...
 !
 ! === Open file for output, write headers and some data ===
 ! Check required disk space: TODO must add nsppol in the list of input variables
 ...
 if (MPI_enreg%me==0) then
  ...
  if (Dtset%accesswff==0) then
   ...
   ! We currently se the dataset symmetries, as defined in the Hdr structure
...
!!$   dtset_cpy%tnons(:,1:nsym2) = tnons2(:,1:nsym2)
  ...
  ! TODO write PAW correctly as well as correct indexing 
  ! Write vkbsign
  ! FIXME : it seems that the present version of outkss deals only with
...
if (Psps%usepaw==0) then
   ...
   if (Psps%usepaw==1) call not_implemented()
...
&       mgfft,MPI_enreg,1,Dtset%ngfft,npw_k,npw_k,n4,n5,n6,option,Dtset%paral_kgb,tim_fourwf,weight)
       if (nspinor==2) then
        pwave_so(:,1:npw_k)=pwave(:,npw_k+1:2*npw_k)
        call fourwf(cplex,vlocal,pwave_so,subghg_so,work,gbound,gbound,istwf_k,kg_k,&
&        kg_k,mgfft,MPI_enreg,1,Dtset%ngfft,npw_k,npw_k,n4,n5,n6,option,Dtset%paral_kgb,tim_fourwf,weight)
...
#if defined MPI
   end if ! k-point+spin parallelism
#endif
 !
 ! === Transfer data between procs ===
! FIXME communicate cprj
#if defined MPI
   bufsz=nbandksseff/bufnb;bufrt=nbandksseff-bufnb*bufsz
   nband_k=Dtset%nband(ikpt+(isppol-1)*nkpt)

   if ((MPI_enreg%me==0).or.(MPI_enreg%proc_distrb(ikpt,1,isppol)==MPI_enreg%me)  ) then
    if ((MPI_enreg%me==0).and.(MPI_enreg%proc_distrb(ikpt,1,isppol)/=MPI_enreg%me)) then
...
&     MPI_enreg%proc_distrb(ikpt,1,isppol),eigvec(:,:,bufnb*bufsz+1:bufnb*bufsz+bufrt),0,spaceComm,ierr)
    end if
...
&     '  with respect to the conjugated gradient values.'
   ...
   ! FIXME in case of parallel execution we have to collect Cprj before reaching this point 
...
#if defined MPI
   if (MINVAL(ABS(MPI_enreg%proc_distrb(ikpt,:,isppol)-MPI_enreg%me))==0) k_index=k_index+npw_k*nband_k*nspinor
#else
