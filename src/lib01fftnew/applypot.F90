...
&        max1o,max2o,max3o,m1o,m2o,m3o,nproc,iproc,paral_kgb,pot,zf)
          use defs_basis
          use defs_fftdata
...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 use interfaces_lib01fftnew, except_this_one => applypot
...
! real space input        
        ...
        integer :: max1o,max2o,max3o,m1o,m2o,m3o,nproc,iproc
...
!$omp critical
        allocate(btrig1(2,n1),ftrig1(2,n1),after1(mdata),now1(mdata),before1(mdata), &
                 btrig2(2,n2),ftrig2(2,n2),after2(mdata),now2(mdata),before2(mdata), &
                 btrig3(2,n3),ftrig3(2,n3),after3(mdata),now3(mdata),before3(mdata), &
...
! communication scheduling
          if ( (npr.eq.4 .and. iam.eq.0 .and. lock.eq.0) .or.  &
              ...
              npr.eq.1                                 ) then
!        write(6,'(a,6(x,i3))') 'Applypot 1 ALLTOALL',nproc,iproc,npr,iam,lock,idat
#if defined MPI
  if(paral_kgb == 1) then
        if (idat.le.ndat) &
        ...
        lock=lock+1
          ...
          lock=mod(lock,2*npr)
!        write(6,'(a,6(x,i3))') 'new lock 1', nproc,iproc,npr,iam,lock
!$omp   flush(lock)
          else
          goto 11
          end if
...
! communication scheduling
        if ( (npr.eq.4 .and. iam.eq.0 .and. lock.eq.6) .or.  &
             ...
             (npr.eq.2 .and. iam.eq.1 .and. lock.eq.1) .or.  &
              npr.eq.1                                 ) then
!        write(6,'(a,6(x,i3))') 'Applypot 2 ALLTOALL',nproc,iproc,npr,iam,lock,idat
#if defined MPI
  if(paral_kgb == 1)then
        if (idat.le.ndat) &
        ...
        lock=lock+1
          ...
          lock=mod(lock,2*npr)
!        write(6,'(a,6(x,i3))') 'new lock 2', nproc,iproc,npr,iam,lock
!$omp   flush(lock)
          else
          goto 22
          end if
