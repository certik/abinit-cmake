...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2008 ABINIT group (PhG,XG)
...
!!
!! CHILDREN
!!      asrif9,canct9,ewald9,matr3inv,q0dy3
!!
!! SOURCE
...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 ...
 use interfaces_17ddb, except_this_one => hybrid9
...
!scalars
 ...
 integer :: chk,d1,d2,flgr,genat1,genat2,ia,ib,id1,id2,index,irpt,isrd,jj,kk,mu
 integer :: nbr,nddd,nsrd,nu,option,sumg0
 real(dp) :: dd,dd2,detdlt,detdlt2,norm2,normr2,rsq,rsq2
 logical :: ex
!arrays
 ...
 real(dp) :: invdlt(3,3),invdlt2(3,3),pos1(5,3),pos2(3),qpt(3),ra(3),rb(3)
...
!+++++++++++++++++++++++++++++++++++++++++
 ...
 open(unit=hyun,file='modifs.ddb',status='old',form='formatted')

 read(hyun,*) nsrd
 if (nsrd<0)then
  write(06, '(a,i8,a)' )&
&  'hybrid9 : nsrd is',nsrd,', which is lower than 0 => stop'
  stop
 end if
 if (nsrd>msrd)then
  write(06, '(a,i8,a,a,i8,a)' )&
&  ' hybrid9 : nsrd is',nsrd,', which is larger than',ch10,&
&  msrd,' , the maximum number allowed => stop'
 ...
 if (nddd<0)then
  write(06, '(a,i8,a)' )&
&  ' hybrid9 : nddd is',nddd,', which is lower than 0 => stop'
  stop
 end if
 if (nddd>mddd)then
  write(06, '(a,i8,a,a,a)' )&
&  ' hybrid9 : nddd is',nddd,', which is not',ch10,&
...
!  identify the R-vector
   ...
   if (flgr==0)then
    write(6, '(a,a,a,i8,a)' )&
&    ' hybrid9 : not able to identify the',ch10,&
...
&   qpt,rmet,rprim,sumg0,ucvol,xred,zeff2)
   if (asr==1.or.asr==2) then
    option=asr
    call q0dy3(natom,dyew2q0,dyew,option)
   else if (asr==0) then
    dyew2q0(:,:,:)=0.0_dp
   end if
  end if
...
&        /dd**5/sqrt(detdlt)
      ...
      if (ia/=ib)then
       write(6,*)' hybrid9 : interatomic distance vanishes ',&
...
&        /dd2**5/sqrt(detdlt2)
      ...
      if (ia/=ib)then
       write(6,*)' hybrid9 : inter-atomic distance vanishes ',&
&       ' Check the input for the following atoms :'
       write(6,*)ia,ib
       stop
      end if
...
! Reimpose the ASR on the ifc that have been modified...
  if(asr>0)then
   write(6,*)' hybrid9 : enter asrif9 '
   call asrif9(asr,atmfrc,natom,nrpt,rpt,wghatm)
   write(6,*)' hybrid9 : exit asrif9 '
  end if
 end if
