...
&    '  headform is ',headform,', while it should be 23, 34, 40, 41, 42, 44, 53 or 56.',ch10,&
...
! Reading the Rhoij tab if the PAW method was used  -----------------------
  if (hdr%usepaw==1) then
   ...
   if ((headform>=44).and.(headform<56)) then
    ...
    bsize=sum(nsel56);allocate(ibuffer(bsize),buffer(bsize*hdr%nspden*cplex))
    ii=0;jj=0
    read(unitfi) ibuffer(:),buffer(:)
    do iatom=1,hdr%natom
...
!ENDDEBUG
  ...
  if (hdr%usepaw==1) then
   ...
   write(unitfi) (nsel56(iatom),iatom=1,hdr%natom),cplex
   bsize=sum(nsel56);allocate(ibuffer(bsize),buffer(bsize*hdr%nspden*cplex))
   ii=0;jj=0
   do iatom=1,hdr%natom
    nselect=nsel56(iatom)
