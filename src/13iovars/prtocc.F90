...
!!  mxmband=maximum numbers of bands
!!  mxnkpt=maximum numbers of k-points
!!  mxnsppol=maximum numbers of spin polarisation
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: iout,mxmband,mxnkpt,mxnsppol,ndtset_alloc,prtvol_glob
!arrays
...
!Local variables-------------------------------
 character(len=*), parameter :: f_occ    ="(1x,a9,1x,(t13,6f10.6))"
 character(len=*), parameter :: f_occa   ="(1x,a9,a,1x,(t13,6f10.6))"
...
!scalars
 ...
 integer :: multi_nkpt,multi_nsppol,multi_occopt,nban,nkpt,nkpt_eff,nsppol
 integer :: occopt,print,tnkpt
...
!multi_occopt==0 and multi_nband==0
 ...
 if(multi_occopt==0 .and. multi_nband==0) then
  nban=sum(dtsets(1)%nband(1:nsppol*dtsets(1)%nkpt))
  multi=0
  if(ndtset_alloc>1)then
   do idtset=1,ndtset_alloc
    if(dtsets(idtset)%iscf/=-2)then
...
!     agree within more than 6 digits
      if(abs(results_out(1)%occ(iban)-results_out(idtset)%occ(iban))>tol8)multi=1
     end do
    end if
   end do
  end if
...
! The quantity of data to be output vary with occopt
  if(occopt>=2)then
   ...
   do isppol=1,nsppol
    do ikpt=1,nkpt_eff
     ikpsp=ikpt+nkpt*(isppol-1)
     nban=dtsets(generic)%nband(ikpsp)
     if(ikpsp==1)then
      write(iout, '(1x,a9,1x,(t13,6f10.6))' )&
&      token,results_out(generic)%occ(iban:iban+nban-1)
     else
      write(iout, '((t13,6f10.6))' )results_out(generic)%occ(iban:iban+nban-1)
     end if
     iban=iban+nban
    end do
...
!  The number of bands is identical for all k points and spin
   ...
   write(iout, '(1x,a9,1x,(t13,6f10.6))' )&
&   token,results_out(generic)%occ(1:nban)
!  if occopt==1, the occ might differ with the spin
   if(nsppol/=1 .and. occopt==1)then
    write(iout,'((t13,6f10.6))')results_out(generic)%occ(nban*nkpt+1:nban*nkpt+nban)
   end if
  end if
 end if
    ...
    if(occopt>=2)then
     ...
     do isppol=1,dtsets(idtset)%nsppol
      do ikpt=1,nkpt_eff
       ...
       if(ikpsp==1)then
        write(iout, '(1x,a9,a,1x,(t13,6f10.6))' )&
&        token,appen,results_out(idtset)%occ(iban:iban+nban-1)
       else
        write(iout, '((t13,6f10.6))' )results_out(idtset)%occ(iban:iban+nban-1)
       end if
       iban=iban+nban
      end do
      if(tnkpt==1) write(iout,'(16x,a)' ) &
...
!    The number of bands is identical for all k points and spin
     ...
     write(iout, '(1x,a9,a,1x,(t13,6f10.6))' )&
&     token,appen,results_out(idtset)%occ(1:nban)
!    if occopt==1, the occ might differ with the spin
     if(dtsets(idtset)%nsppol/=1 .and. dtsets(idtset)%occopt==1)then
      write(iout, '((t13,6f10.6))' ) &
&      results_out(idtset)%occ(nban*dtsets(idtset)%nkpt+1:nban*dtsets(idtset)%nkpt+nban)
     end if
    end if
   end if
!  Endloop on idtset
