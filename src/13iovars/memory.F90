...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 ...
 use interfaces_13iovars, except_this_one => memory
...
!scalars
 ...
 integer,intent(in) :: mqgrid_vl,n1xccc,natom,nfft,nfftdiel,nfftf,nkpt,npsp
 integer,intent(in) :: npulayit,npwdiel,nspden,nspinor,nsppol,nstep,nsym,ntypat
 integer,intent(in) :: occopt,optforces,option,optstress,pawmixdg,pawnhatxc,pawstgylm
...
!PAW: store useful dims
 if (usepaw==1) then
  allocate(basis_size(npsp),l_size(npsp),lmn_size(npsp),&
&  lmn2_size(npsp),mesh_size(npsp),shape_type(npsp),&
&  pawver(npsp),rshp(npsp))
  do ii=1,npsp
...
!irrzon  (gstate)
 if(nsym/=1)then
  cfft(4)=2*(nspden/nsppol)     ; dttyp(4)=4
 end if
!ylm (gstate)
...
!phnons (in gstate)
 if(nsym/=1)then
  cfft(10)=2*(nspden/nsppol)    ; dttyp(10)=8
 end if
!xccc1d (in driver)
...
! additional arrays
  ...
  do ii=1,ntypat
   cadd(16)=cadd(16)+nattyp(ii)*lmn2_size(ii)*nspden        ! Rhoij and related data
   cadd(17)=cadd(17)+nattyp(ii)*(2+lmn2_size(ii))*nspden    ! (rhoijselect, ...)
  end do
 end if
...
!SCF history (if selected)
 if (abs(iprcch)==5.or.abs(iprcch)==6) then              ! scf_history...
  ...
  if (usepaw==1) then
   do ii=1,ntypat
    cadd(19)=cadd(19)+2*nattyp(ii)*lmn2_size(ii)*nspden     ! %pawrhoij()%rhoijp
...
! dielinv, susmat
  cadd(23)=4*(npwdiel*nspden)**2; dttyp(23)=8
 end if
...
&  *pawstgylm*(3*l_size(ii)**2+3*optstress)
   ...
   cadd(25)=cadd(25)+nattyp(ii)*lmn2_size(ii)*nspden                     ! rhoijres
   cadd(25)=cadd(25)+nattyp(ii)*lmn2_size(ii)*nspden*n_fftgr*mffmem      ! f_paw
  end do
...
!PAW:cprj
 if (usepaw==1) then
  dttyp(58)=8
  do ii=1,ntypat
   cadd(58)=cadd(58)+2*nattyp(ii)*nkpt*nspinor*mband*nsppol*lmn_size(ii)
  end do
 end if
...
!-------------------------------------------------------------------------
 if (usepaw==1) deallocate(basis_size,l_size,lmn_size,lmn2_size,mesh_size,shape_type,rshp)
