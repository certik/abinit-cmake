...
!write the phonon frequencies on unit 6
 write(6, '(a)' )' '
 write(6, '(a,a)' )' phonon wavelength (reduced coordinates) , ', &
...
& ' ',(qphon(i),i=1,3),qphnrm
 do jj=1,3*natom,5
  if (3*natom-jj<5) then
   write(6, '(5es14.6)') (phfrq(ii),ii=jj,3*natom)
  else
   write(6, '(5es14.6)') (phfrq(ii),ii=jj,jj+4)
  end if
 end do
...
&    '  Mode number ',imode,'   Energy',phfrq(imode)
   ...
   if(phfrq(imode)<1.0d-5)then
    write(6, '(a)' )' Attention : low frequency mode.'
    write(6, '(a)' )'   (Could be unstable or acoustic mode)'
    if(iout>=0)then
   ...
   do ii=1,natom
    ...
    write(6, '(i4,3es16.8)' ) ii,vectr(:)
    write(6, '(4x,3es16.8)' )    vecti(:)
    if(iout>=0)then
