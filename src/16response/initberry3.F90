...
!Initialization of efield_type variables
 ...
 allocate(dtefield%ikpt_dk(nkpt,9,3))
 allocate(dtefield%cgindex(nkpt,nsppol*2))
...
!at k +- dk
 ...
 allocate(kg_tmp(3,max(mpw,mpw1)*mkmem),kpt1(3,nkpt))
 allocate(npwarr_tmp(nkpt),npwtot(nkpt))
...
!   Set up the basis sphere of plane waves at kpt1
    kg_tmp(:,:) = 0
    call kpgio(ecut_eff,dtset%exchn2n3d,gmet,dtset%istwfk,kg_tmp,tmpfil,&
...
&     dtset%kptns(:,ikpt1))
 ...
 deallocate(kg_tmp,kpt1,npwarr_tmp,npwtot)
