...
!!
!! INPUTS
!!  l_max= max. value of ang. momentum l;  Gaunt coeffs up to
...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 use interfaces_13paw, except_this_one => realgaunt
...
!no_abirules
 type coeff_type    ! Gives the matrix cc where Sl=cc*Yl
  real(dp), pointer :: cc(:,:,:)
 end type coeff_type
 type(coeff_type), allocatable :: coeff(:)
!************************************************************************
...
!------------------------------------------------
 ...
 do ll=1,4*l_max-3
  allocate (coeff(ll)%cc(2,2*ll-1,2*ll-1))
  coeff(ll)%cc(:,:,:)=zero
  coeff(ll)%cc(1,ll,ll)=one
  do mm=1,ll-1
   coeff(ll)%cc(1,ll+mm,ll+mm)= (-1._dp)**mm/sqrt(2._dp)
   coeff(ll)%cc(1,ll-mm,ll+mm)= ( 1._dp)    /sqrt(2._dp)
   coeff(ll)%cc(2,ll+mm,ll-mm)=-(-1._dp)**mm/sqrt(2._dp)
   coeff(ll)%cc(2,ll-mm,ll-mm)= ( 1._dp)    /sqrt(2._dp)
  end do
 end do
...
!        ------------------------------------------------
         realgnt_tmp=zero
         do mm1=-l1,l1
          c11=coeff(l1+1)%cc(1,l1+mm1+1,l1+m1+1)
          c12=coeff(l1+1)%cc(2,l1+mm1+1,l1+m1+1)
          do mm2= -lp1,lp1
           c21=coeff(lp1+1)%cc(1,lp1+mm2+1,lp1+mp1+1)
           c22=coeff(lp1+1)%cc(2,lp1+mm2+1,lp1+mp1+1)
           do mm3= -l2,l2
            c31=coeff(l2+1)%cc(1,l2+mm3+1,l2+m2+1)
            c32=coeff(l2+1)%cc(2,l2+mm3+1,l2+m2+1)
