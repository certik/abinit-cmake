...
!! Yet this implementation is based on a variational model.
...
!End of the abilint section
 ...
 implicit none
  ...
  integer :: ifft2,ig,ig1,ig2,ig3,ii,ikg,ikpt,ilm,ilmn,index,ispden,isppol
  integer :: istwf_k,master,matblk,mcg_disk,me,muig,n1,n2,n3,n4,n5,n6,nband_k
  integer :: nfftotf,nkpg,nnlout,npw_k,option,paw_opt,signs,spaceComm,tim_fourwf
  integer :: tim_mkrho,tim_nonlop,tim_rwwf
  integer :: me_distrb
  real(dp),parameter :: alpha32=(3._dp*pi*pi)
  real(dp) :: Z,a,arg,b,c,dfermi,doti,dotr,dummy,dummy2,eeigk,enlk,rdummy1
  ...
  real(dp) :: aa(2),bb(2),bla(nfftf,nspden),cc(2),dummy_strsxc(6),enlout(1)
  real(dp) :: gmet(3,3),gprimd(3,3),kpoint(3),nonlop_dum(1,1),rhodum(1),rmet(3,3)
  real(dp) :: tsec(2),vmean(2),vref(nfftf,nspden),vrefg(2,nfftf,nspden)
  ...
  real(dp),allocatable :: phiout(:,:),resid_k(:),sqrtrhor(:,:)
...
!PATCH prctfw2 // KPT & FFT me-->me_kpt & spaceComm --> comm_kpt
...
& '  New local average of non local potential made from  wfs and Vnl'
 ...
 deallocate(lavnl)
 allocate(vtrialfourier(2,nfftf,nspden),newvoutfourier(2,nfftf,nspden),laplacerhor(nfftf,nspden))
!******************************************************************
...
!!  Compute the real space laplacian of sqrt(rhor)
 allocate(sqrtrhor(nfftf,nspden))
 sqrtrhor(:,:)=(rhor(:,:))**half
...
!******************************************************************
 allocate(vhartr(nfftf))
 allocate(vxc(nfftf,nspden))
 allocate(phi1(nfftf,nspden))
 allocate(phi2(nfftf,nspden))
 allocate(opA(nfftf,nspden))
 allocate(opB(nfftf,nspden))
!sqrtrhor(:,:)=(rhor(:,:))**half
 tim_fourdp=0
 cplex=1
 isign=-1
...
!!$       &  typat,ucvol,bla,xredcp,znucl)
 ...
 write(6,*) 'prctfwf2 (04rsprc) : min of opA',a
 write(6,*) 'prctfwf2 (04rsprc) : min of kxc',b
 write(6,*) 'prctfwf2 (04rsprc) : efermi',efermi
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
&sqrtrhor,&           !density
&ucvol)           !cell volume
 write(6,*) 'prctfwf2 (04rsprc) : total density shift:',a,' this ought to be zero...'
 write(6,*) 'prctfwf2 (04rsprc) : dfermi',dfermi
 if(a /= zero) then
  sqrtrhor(:,:)=sqrt(rhor(:,:))
  cplex=1;
  option=1;
...
& ucvol)          !cell volume
  Z=real(nint(Z),dp)
  sqrtrhor(:,:)=phi1+dfermi*phi2
  rhor(:,:)=rhor(:,:) + two*rhor(:,:)**half*sqrtrhor(:,:)  + sqrtrhor(:,:)**2
  sqrtrhor(:,:)=sqrt(rhor(:,:))
  call dotprod_vn(cplex,& !complex density/pot
& sqrtrhor,&          !the sqrtdensity
...
!write(3339,*) g2cart
!write(0,*) '3'
 deallocate(vhartr,vxc,vresidnew,opA,opB,phi1,phi2)
!write(0,*) '4'
...
!----------------------------------------------------------------------------------------------------------
!last FREE
 write(6,*) 'prctfwf2 (04rsprc) : FREEING THE REMAINING OF MEMORY FROM PRCTFW'
 deallocate(lavnlfft)
!deallocate(deltaW,&
 deallocate(newvout)
