...
!!
!! CHILDREN
!!      cgpr,frskerker1__end,frskerker1__init,laplacian
!!
!! SOURCE
...
!arrays
 ...
 real(dp),intent(in) :: base(nfft),dielar(7),gprimd(3,3),rprimd(3,3)
 real(dp),intent(in) :: vresid(nfft,nspden),xred(3,natom)
 real(dp),intent(out) :: vrespc(nfft,nspden)

!Local variables-------------------------------
!scalars
 integer :: c1,c2,cplex,i1,i2,i3,iatom,iatom27,ifft,ispden,n1,n2,n3,natom27
 real(dp) :: base_delta,base_max,base_min,base_shift,core,dielng,diemac,diemix
 real(dp) :: dr,l1,l2,l3,l4,maxg2,ming2,p0=1_dp,p1=0_dp,p2=26.7288_dp
 real(dp) :: p3=-236.423_dp,p4=711.185_dp,p5=-1071.08_dp,p6=881.438_dp
 real(dp) :: p7=-390.626_dp,p8=67.7794_dp,r,rdummy1,rdummy2,rmin,x,xp,xp2,xr,y
 real(dp) :: yr,zr
 logical :: new_prc_func
!arrays
 real(dp),parameter :: identity(4)=(/one,one,zero,zero/)
 real(dp) :: base_cp(nfft),bla(nfft/351),buffer(nfft,nspden)
 real(dp) :: buffer2(nfft,nspden),deltaW(nfft,nspden),dvstar(nfft,nspden)
 real(dp) :: g2cart(nfft),gvrespc(2,nfft,nspden),ldvstar(nfft,nspden)
 real(dp) :: lvres(nfft,nspden),mat(nfft,nspden),qphon(3),rdiel(nfft)
 real(dp) :: xcart(3,natom),xcart27(3,natom*27)
