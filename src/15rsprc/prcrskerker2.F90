...
!! differing from prcrscgres by the
...
!scalars
 ...
 real(dp) :: C1,C2,DE,core,dielng,diemac,diemix,doti,dr,l1,l2,l3,l4,r,rdummy1
 real(dp) :: rdummy2,rmin,x,xp,xp2,xr,y,yr,zr
!arrays
 real(dp),parameter :: identity(4)=(/1.0_dp,1.0_dp,0.0_dp,0.0_dp/)
 real(dp) :: V1(nfft,nspden),V2(nfft,nspden),bla(nfft/351),buffer(nfft,nspden)
 real(dp) :: buffer2(nfft,nspden),deltaW(nfft,nspden),dvstar(nfft,nspden)
 real(dp) :: ldvstar(nfft,nspden),lvres(nfft,nspden),mat(nfft,nspden),qphon(3)
 real(dp) :: rdiel(nfft),rdielng(nfft),rdiemac(nfft),xcart(3,natom)
...
!    if(iatom==4) write(0,*) xcart27(:,iatom27)
    end do
   end do
  end do
 end do
...
! deltaW(:,ispden)= -diemix*(((rdielng(:))**2)*deltaW(:,ispden))
 end do
