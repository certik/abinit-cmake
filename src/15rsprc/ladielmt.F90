...
!arrays
 ...
 real(dp) :: lavnlr(dtset%nfft,dtset%nspden),nonlop_dum(1,1),qphon(3)
...
!step 1
 allocate(bigdielmat(2,dtset%nfft,dtset%nfft),mask(npwdiel))
...
!step 2
 allocate(bigrdielmat(2,dtset%nfft,dtset%nfft))
 do ipw2=1,npwdiel
...
&  1,mpi_enreg,dtset%nfft,dtset%ngfft,dtset%paral_kgb,0)
 ...
 deallocate(bigdielmat)
!step 3
 allocate(rdielmat(2,n4,n5,n6,n4,n5,n6))
 do i3=1,n3
  do i2=1,n2
   do i1=1,n1
...
!allocate(vlocal(n4,n5,n6,nvloc);
 allocate(kg_k(3,dtset%mpw),cwavef(2,dtset%mpw*nspinor), vnlcwavef(2,dtset%mpw*nspinor))
 allocate(wfraug(2,n4,n5,n6),vnlwfraug(2,n4,n5,n6),lavnl3d(n4,n5,n6))
...
!Not the same meaning for norm-cons. psps and paw
 ...
 allocate(gs_hamk%ekb(psps%dimekb,psps%ntypat,nspinor**2))
 ...
 allocate(gs_hamk%sij(gs_hamk%dimekb1,psps%ntypat))
...
&    psps%pspso,&         ! pspso(psps%ntypat)=spin-orbit characteristic for each atom type
...
&     npw_k,1,n4,n5,n6,0,dtset%paral_kgb,tim_fourwf,dummy)
!    call fourwf(1,dummyt3,vnlcwavef,dummyt2,vnlwfraug,gs_hamk%gbound,gs_hamk%gbound,&
!    &      istwf_k,kg_k,kg_k,dtset%mgfft,mpi_enreg,1,gs_hamk%ngfft,npw_k,1,n4,n5,n6,0,dtset%paral_kgb,tim_fourwf,dummy)
...
!  lavnlr(:,ispden) = lavnlr(:,ispden)-min+1.0d0
   ...
   xred2=dtset%xred_orig
   write(filapp,770) 'dielmat_avg'
...
!DEBUG
!write(0,*) "DEBUG lavnl: end of density analysis"
!write(0,*) "DEBUG ================  lavnl ==========================="
!ENDDEBUG
