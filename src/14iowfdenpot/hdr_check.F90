...
!!
!! COPYRIGHT
!! Copyright (C) 2002-2008 ABINIT group (DCA, XG, GMR, FB).
...
!!   (I) the real/complex character of rhoij              (tcplx)
!! For WVL calculations:
!!   (J) the number of wavelets differs                    (twvl)
!!   (K) the space-grid size differs                       (tgrid)
!!
...
!!   (J) the real/complex character of rhoij              (tcplx)
!!
!! TODO
!!
!! PARENTS
...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 ...
 use interfaces_14iowfdenpot, except_this_one => hdr_check
...
!scalars
 ...
 integer :: jj,mu,natom,nelm,nkpt,npsp,nsppol,nsym,ntypat,tatty,tband,tcplx,tdg
...
!that there is complete agreement between the files
 ...
 tatty = 0; tband = 0; tcplx = 0; tdg = 0 ; tecut = 0; tkpt = 0;
...
&   '  disk file rprimd is not parallelepipedic.'
  ...
  if (abs(hdr0%rprimd(1,1) / hdr0%ngfft(1) - hdr %rprimd(1,1) / hdr %ngfft(1)) > tol8) then
   write(message, '(a,a,a,a,F6.4,a,F6.4)' ) ch10,&
...
! Compare real/complex character of rhoij (PAW)
  write(message, '(a,i1,20x,a,a,i1)' )&
&  '  PAW: rhoij%cplex ',hdr %pawrhoij(1)%cplex,'|',&
&  '  PAW: rhoij%cplex ',hdr0%pawrhoij(1)%cplex
  call wrtout(06,message,mode_paral)
  if (hdr%pawrhoij(1)%cplex/=hdr0%pawrhoij(1)%cplex) then
   write(message, '(a,a,a,a,i1,a,i1)' ) ch10,&
&   ' hdr_check: WARNING -',ch10,&
&   '  input rhoij%cplex=',hdr%pawrhoij(1)%cplex,&
&   '  not equal disk file rhoij%cplex=',hdr0%pawrhoij(1)%cplex
   call wrtout(06,message,mode_paral)
   tcplx=1
  end if
...
&      ' hdr_check: WARNING -',ch10,&
&      '  k pt num',ii,' input nband=',hdr%nband(ii),&
...
&   '  ABINIT is not implemented for wavelet resolutions different from 2.'
  ...
  write(message,'(a,30x,a,a)') '  nwvlres:','|', &
&  '  nwvlres:'
  call wrtout(06,message,mode_paral)
  write(message,'(a,2I6,24x,a,a,2I6)') '    ',hdr%nwvlarr, '|', &
&  '    ', hdr0%nwvlarr
  call wrtout(06,message,mode_paral)
  do ii=1,2
...
!Compare typat(natom)
 if (hdr%natom==hdr0%natom) then
  ...
  do istart = 1,natom,12
   ...
   nelm = istop - istart + 1
   call mk_hdr_check_fmt(nelm,typfmt)
   write(message,fmt=typfmt) hdr%typat(istart:istop),&
&   '|',hdr0%typat(istart:istop)
   call wrtout(06,message,mode_paral)
  end do
  do ii=1,natom
...
!Compare so_psp(npsp)
 if (hdr%npsp==hdr0%npsp) then
  ...
  do istart = 1,npsp  ,12
   ...
   nelm = istop - istart + 1
   call mk_hdr_check_fmt(nelm,typfmt)
   write(message,fmt=typfmt) hdr%so_psp  (istart:istop),&
&   '|',hdr0%so_psp  (istart:istop)
   call wrtout(06,message,mode_paral)
  end do
  do ii=1,npsp
...
!Compare istwfk(nkpt)
 if (hdr%nkpt==hdr0%nkpt) then
  ...
  do istart = 1,nkpt,12
   ...
   nelm = istop - istart + 1
   call mk_hdr_check_fmt(nelm,typfmt)
   write(message,fmt=typfmt) hdr%istwfk(istart:istop),&
&   '|',hdr0%istwfk(istart:istop)
   call wrtout(06,message,mode_paral)
  end do
  do ii=1,nkpt
...
&   hdr%kptns(:,ii),'|',hdr0%kptns(:,ii)
   ...
   if(ii>nkpt_max)then
    write(message,'(a)')&
&    '  The number of printed k points is sufficient ... stop writing them.'
    call wrtout(06,message,mode_paral)
    exit
   end if
  end do
...
&    ' hdr_check: WARNING -',ch10,&
&    '  k pt num',ii,', input kpt=',hdr%kptns(:,ii),ch10,&
...
&    ' hdr_check: WARNING -',ch10,&
&    '  k pt num',ii,', input weight=',hdr%wtk(ii),ch10,&
...
!Compare tnons(3,nsym)
 if (hdr%nsym==hdr0%nsym) then
  ...
  do isym=1,nsym
   write(message,'(2x,3f12.7,2x,a,2x,3f12.7)') &
&   hdr%tnons(:,isym),'|',hdr0%tnons(:,isym)
   call wrtout(06,message,mode_paral)
  end do
...
!Compare znucltypat(ntypat)
 if (hdr%ntypat==hdr0%ntypat) then
  ...
  do istart = 1,ntypat,6
   ...
   write(message,fmt=zatfmt) hdr%znucltypat(istart:istop),&
&   '|',hdr0%znucltypat(istart:istop)
   call wrtout(06,message,mode_paral)
  end do
...
!DEBUG
!write(6,*)' hdr_check : consistency of floating point data checked '
!stop
!ENDDEBUG
...
!DEBUG
!write(6,*)' hdr_check : consistency of psp header checked '
!stop
!ENDDEBUG
...
!DEBUG
!write(6,*)' hdr_check : resdim and atom position checked '
!stop
!ENDDEBUG
...
& (hdr%usepaw == 1 .and. &
& (tpaw /= 0 .or. tlmn /= 0 .or. tcplx /= 0 .or. tdg /= 0)) .or. &
& (hdr%usewvl == 1 .and. &
& (tatty /= 0 .or. tband /= 0))
...
& (hdr%usepaw == 1 .and. &
& (tpaw /= 0 .or. tlmn /= 0 .or. tcplx /= 0 .or. tdg /= 0))
...
! Tests for a restart in the framework of the PAW method
  if (hdr%usepaw/=0 .or. hdr0%usepaw/=0) then
   if (tpaw /= 0 .or. tlmn /= 0 .or. &
&   tcplx /= 0) restartpaw=0
   if (restartpaw == 0) then
    write(message,'(8x,a)') &
&    'Critical differences for a restart within PAW method:'
    call wrtout(06,message,mode_paral)
    if ( tpaw /= 0 ) then
     write(message, '(8x,a)' ) &
&     '* the use of the PAW method'
     call wrtout(06,message,mode_paral)
    else
     if(tlmn/=0)then
      write(message, '(8x,a)' ) &
&      '* the number of lmn elements for the paw basis'
      call wrtout(06,message,mode_paral)
     end if
     if(tcplx/=0)then
      write(message, '(8x,a)' ) &
&      '* the real/complex character of on-site occupancies (rhoij)'
      call wrtout(06,message,mode_paral)
     end if
    end if
   else if (tdg/=0) then
