...
!!
!! COPYRIGHT
!! Copyright (C) 2001-2008 ABINIT group (XG, DRH)
...
!This section has been created automatically by the script Abilint (TD). Do not modify the following lines by hand.
 use interfaces_13xc, except_this_one => mkvxcgga3
...
!arrays
 ...
 real(dp),intent(in) :: gmet(3,3),gprimd(3,3),kxc(nfft,nkxc),qphon(3)
...
!write(6,'(a)') ' mkvxcgga3 :  '
!write(6,'(a)') '   ispden   ir     rho1now(ir,ispden,1:4)'
!do ispden=1,2
!do ir=1,nfft
...
!end do
!write(6,'(a)') ' mkvxcgga3 :  '
!write(6,'(a)') '   ispden   ir     rhortmp(ir,ispden,1:4)'
!do ispden=1,2
!do ir=1,nfft
!if(ir<=11 .or. mod(ir,301)==0)then
!write(message,'(2i5,a,4es14.6)')ispden,ir,' ',&
...
!end do
!write(6,'(a)') ' mkvxcgga3 :  '
!write(6,'(a)') '   ispden   ir     kxc(ir,1:4)'
!do ir=1,nfft
!if(ir<=11 .or. mod(ir,301)==0)then
...
!end do
!write(6,'(a)') ' mkvxcgga3 :  '
!write(6,'(a)') '   ispden   ir     kxc(ir,5:8)'
!do ir=1,nfft
!if(ir<=11 .or. mod(ir,301)==0)then
!write(message,'(i5,a,4es14.6)')ir,' ',&
!&   kxc(ir,5:8)
...
!write(6,'(a)') ' mkvxcgga3, before xcpot :  '
!write(6,'(a)') '   ispden   ir     dnexcdn '
!do ispden=1,2
!do ir=1,nfft
!if(ir<=11 .or. mod(ir,301)==0)then
!write(message,'(2i5,a,4es14.6)')ispden,ir,' ',&
...
!end do
!end do
!write(6,'(a)') '   ispden   ir     rho1now(ir,1:4)'
!do ispden=1,2
!do ir=1,nfft
!if(ir<=11 .or. mod(ir,301)==0)then
!write(message,'(2i5,a,4es14.6)')ispden,ir,' ',&
...
!write(6,'(a)') ' mkvxcgga3 :  '
!write(6,'(a)') '   ispden   ir     rhor1tmp(ir)     vxc1tmp(ir) '
!do ispden=1,2
!do ir=1,nfft
...
!end do
!end do
!write(6,*)' mkvxcgga3 : exit '
!write(6, '(a,4es16.6)' )' ex(1:4)=',ex(1:4)
!write(6, '(a,4es16.6)' )' ex(5:8)=',ex(5:8)
!write(6, '(a,4es16.6)' )' ec(1:4)=',ec(1:4)
!write(6, '(a,4es16.6)' )' ec(5:7)=',ec(5:7)
!write(6,*)' sum=',sum(ex(1:8))+sum(ec(1:7))
!stop
!ENDDEBUG
