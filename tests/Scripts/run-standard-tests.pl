#
# Script for running various tests of abinit, newsp, mrgddb, anaddb, etc ...
...
# Copyright (C) 2004-2008 ABINIT group (LSi)
...
# 	where serie can be fast, v1, v2, v3, v4, v5, physics, cpu or tutorial; if omitted, the tests
...
#  run-standard-tests fock ../src/main /home/me/abinit/test fast  >& log_file        run all tests in fast on hilbert
#  run-standard-tests hilbert ../src/main /home/me/abinit/test v1 1  >& log_file        run only test 1 in v1 on hilbert
#  run-standard-tests hilbert ../src/main /home/me/abinit/test v2 3 end  >& log_file    run tests from case_3 to the end in v2 on hilbert
#  run-standard-tests hilbert ../src/main /home/me/abinit/test tutorial 8 22  >& log_file    run tutorial tests between 8 and 22 included.
#

$abinit_cmdpar = $ENV{'abinit_cmdpar'};
...
@Series = ('fast','v1','v2','v3','v4','v5','physics','cpu','tutorial','tutorespfn','etsf_io','etsf_xc','bigdft','wannier90');
$TestDir = '.';		# default is current directory
# abinis processing options:
...
$fldxml = 3;	# run diff/fldiff also on the xml file tNNN.o_CML.xml
...
# I know, the following is ugly! (YP)
$abinit_nightlydir = "$abinit_bindir/../nightly";
$top_testdir = $ARGV[2];
$argptr = 3;
...
# since unlink <file*> fails on some versions of perl for DOS, let's use del
  ...
  elsif ( $UNkernel eq 'CYGWIN_NT' || $UNkernel eq 'MINGW32_NT' ) {
...
# under other unices
    ...
    if ($UNkernel eq 'CYGWIN_NT') {
# since perl for Windows NT is not a PGI Workstation command (cygwin32) but a
# DOS module, calling an abinit module or the fldiff perl script by means of the# "system" function requires DOS conventions
      $PERLSLASH = $DOSSLASH;	# subdirectory delimitor in DOS file paths
      }
    else {
...
$WORK_DIR = ',,'.$MACH.'_'.$OStype.'_'.$YYYYMMDD;
...
$CHKREPORT = ',,chkinabi.report';   # report file for chkinabi commands
# try to rename report file if it already exists
$rc = 0;
if (-f $CHKREPORT) {
        for ($i=1;$i <= 9;$i++) {
                $CHKREP2 = ",,old_chkinabi$i".'.report';
                if (! -f $CHKREP2) {
...
$CODE_CPU = &perlpath("$abinit_bindir/abinis$XSUFX");  # sequential version for cpu tests
$CODE_CPU = "$timeout_cmd$CODE_CPU";
$CODE_NEWSP = &perlpath("$abinit_bindir/newsp$XSUFX");	# codename for newsp
$CODE_NEWSP = "$timeout_cmd$CODE_NEWSP";
...
#$CODE_PAR = &perlpath("$abinit_cmdpar");	# codename for parallel version WARNING XG051130 DOES NOT WORK
$CODE_PAR = '/usr/local/mpi-pgi4/bin/mpirun -np 2 -machinefile sleepy.pcpm.ucl.ac.be:2 ../../../src/main/abinip' ;      # run parallel version on sleepy
$CODE_PAR = "$timeout_cmd$CODE_PAR";

$INPUTDIR = &transpath("$top_testdir/$TestDir/Input");		# input directory
$REF = &transpath("$top_testdir/$TestDir/Refs");		# reference directory
$PSPS = &transpath("$top_testdir/Psps_for_tests");	# pseudopotential directory  
...
# 	abinis | seq	exercise sequential version
#		aim
#		ana | anaddb
#		band2eps
#		chi
...
while (<CONF>) {
	...
	if ($CurTest eq '') {
		if ($testID eq $FIRST || $FIRST eq 'first') {
			...
			$option = 0;		# option for abinis tests
...
# ****************************************************************** 
	...
	if ($prgcmd eq 'seq' || $prgcmd eq 'abinis') {
		&doseqtest($CurTest,$P1,$P2,$P3,$P4,$P5,$P6,$P7,$P8,$P9,$P10,$P11,$P12);
		}
	elsif ($prgcmd eq 'newsp') {
		&newsptest($CurTest,$P1,$P2,$P3);
		}
		...
		&domrgtest($CurTest,$ParmString);
                ...
                $ix = index($_,$P1);
                $ParmString = substr($_,$ix);
                &domrgscrtest($CurTest,$ParmString);
		}
...
# try pspI=
	$hit2 = m/psp([1-9])=(.*)/;
	if ($hit2 && ($prgcmd eq 'seq' || $prgcmd eq 'abinis' || $prgcmd eq 'aim')) {
...
# try GEO=
	$hit3 = m/GEO=(.*)/;
	if ($hit3 && ($prgcmd eq 'seq' || $prgcmd eq 'abinis')) {
...
# try fldiff option for abinis, aim, cut3d, mrgddb, mrgscr or lwf
	return if ($prgcmd ne 'seq' && $prgcmd ne 'abinis' && $prgcmd ne 'cut3d' && $prgcmd ne 'aim' &&
		$prgcmd ne 'lwf' && $prgcmd ne 'macroave' && $prgcmd ne 'anaddb' && $prgcmd ne 'conducti' &&
	...
	return if ($prgcmd ne 'seq' && $prgcmd ne 'abinis');
# try abinis option
	if ($hit5) {
		if ($1 eq '-nofld') {
			$fldflag = $fldNorun;	# don't run the fldiff script
...
# command may fail:
		...
		if ($rc != 0) {
			...
			print "File $path copied and unzipped";
...
# Use the floating diff script to get a compact report on the Run
	...
	print "$FLDIFF $deffldopt $fldopt $result $reference" if ($debug >= 2);
	system ("$FLDIFF $deffldopt $fldopt $result $reference $label >> $FLDREPORT");
	return;
	}
# ****************************************
sub doseqtest {
...
# Remove existing files related to this test case:
	$RUNfile = 'abfiles.run';		# files file for abinit
	...
	unlink($RUNfile,$difffn,$difibmfn,$logfn,$outfn);
...
# Run the test case
	return if ($debug >= 3);
	print "File $RUNfile created, starting abinit";
	if ($OStype eq 'MacOS') {
...
# Compare with reference files
	...
	print "Finished abinit, comparing $outfn with $REFoutfn";
...
# handle o_DOS file for v1 case #07
	if ($fldflag == $fldDOS) {
		...
		print "Comparing $oDOSfile with $REFoDOS";
...
# handle DS3_DOS_AT000x file for tests v4 #35,38
	if ($DS3ATfn ne '') {
		...
		print "Comparing $DS3ATfn with $REFDS3AT";
...
# compare with reference file
		...
		print "Comparing $GEOfn with $REFGEOfn";
...
# v1 Case 40, 41 and 42: also take into account the xml files
	if ($fldflag == $fldxml) {
		$cmlfn = $fn4.'_CML.xml';	# t40o_CML.xml
		$REFcmlfn = &transpath("$REF/$cmlfn");
		print "Comparing $cmlfn with $REFcmlfn";
		$difffn = 'diff.'.$cmlfn;         # diff.t40o_CML.xml
		unlink($difffn);
		system ("$DIFF_COMMAND $cmlfn $REFcmlfn > $difffn");
		&dofldiff("Case_$TID : CML.xml file",$cmlfn,$REFcmlfn,$fldopt);	# compare floating point numbers
		}
...
#
# purpose: exercise newsp
# arguments:
#		$TID = test identification (1 to 3 characters string)
#		$p1,$p2,$p3 = 3 parameters [re]defining the file names (fn1 to fn3) that
# will be put into the "files" file named "newsp.run"; this file will be used
# as input to newsp; the parameters format is: fnI=path
# set default file name(s) derived from $TID:
	$fn1 = '';		# undefined
	$fn2 = '';		# undefined
	$fn3 = "t$TID".'o_WFK';	# e.g. t06o_WFK
...
#
	print "Case_$TID:";
	# fn1 and fn3 may be related to current working dir,
	# but fn2 is linked to input dir.
	$fn2 = &transpath("$INPUTDIR/$fn2");
	print $fn1,$fn2,$fn3 if ($debug >= 2);
# Nothing to do in case 'MACH' is set to chkinabi
        if ($MACH eq 'chkinabi') {
                return ;
                }
	$RUNfile = 'newsp.run';		# files file for newsp
# Remove existing files related to this test case:
	$logfn = 't'.$TID.'.log';		# t06.log
	unlink($RUNfile,$logfn,$fn3);
	# Create "files" file with file names $fn1, ... one per line
	open(FILES,">$RUNfile") || die "Unable to open FILES for test $TID";	
	print FILES &transpath($fn1);	# translate path name when required
	...
	close(FILES);	
# Run the test case
	print "File $RUNfile created, running newsp";
	return if ($debug >= 3);
	if ($OStype eq 'MacOS') {
		system ("$CODE_SEQ < $RUNfile \xb7 $logfn ");
		}
	else {
		system ("$CODE_NEWSP < $RUNfile > $logfn $REDIRECT_ERR");
		}
	return;			# go read next line from configuration file
	}
# ****************************************
...
#   set default file name(s) derived from $TID:
	$fn1 = "$top_testdir/$TestDir/Input/t$TID.in";	# e.g. ../t13.in
	...
	$fn7 = "$top_testdir/$TestDir/Input/t$TID.ddk";	# ../t13.ddk default name for file containing list of ddk filenames
# set new file names according to the parameters
...
# Remove existing files related to this test case:
	...
	$outfn = &transpath($fn2);	# translate path name when required
	unlink($RUNfile,$difffn,$logfn,$outfn);
...
# Create "files" file with file names $fn1, ... one per line
	...
	return if ($debug >= 3);
# Run the test case
	print "File $RUNfile created, starting anaddb";
	if ($OStype eq 'MacOS') {
...
# Compare with reference files
	$REFoutfn = &transpath("$REF/$fn2");
	print "Finished anaddb, comparing $outfn with $REFoutfn";
	system ("$DIFF_COMMAND $outfn $REFoutfn > $difffn");
...
# Create "files" file with file names $fn1, ... one per line
	...
	return if ($debug >= 3);
# Run the test case
	print "File $RUNfile created, starting mrgddb";
	if ($OStype eq 'MacOS') {
...
# Compare with reference files
	...
	print "Finished mrgddb, comparing $outfn with $REFoutfn";
	system ("$DIFF_COMMAND $outfn $REFoutfn > $difffn");
...
# to log file names
# $parms = parameters string following the test ID; the format is:
#	files=fn1,fn2[,...,fnLast] [opt=option]
#	fn1 ... fnLast = list of files for mrgscr; variable $nfile will be
# set to the files count;
#	option is an option for fldiff, see setfnopt for the allowed values
#   $nfile, a standard root output filename and fn1 to fnLast will be put into the
#   "files" file named "mrgscr.run"; this file will be used as input to mrgscr;
#   fn1 to fnLast can reside in the Test directory as normal or g-zipped files,
...
#   set default file name(s) derived from $TID:
	...
	for ($i=0;$i <= $#tokens;$i++) {
        ...
        $nfile = $#mrgfiles+1;
	if ($debug >= 2) {
		print "FN1 $fn1 FILES $nfile @mrgfiles";
		print "fldflag $fldflag fldopt $fldopt";
		}
        if ($nfile <= 1) {
		print 'Error, files list is missing or not coma-separated';
...
# Remove existing files related to this test case:
        $RUNfile = 'mrgscr.run';                # files file for mrgscr
        $outfn = "t$TID.out";           # t14.log
        $difffn = "diff.t$TID";         # diff.t14
        $outscrfn = &transpath($fn1);      # translate path name when required
        unlink($RUNfile,$difffn,$outfn,$outscrfn);
# if a file fn1 ... fnLast does not exist, it is probably to be copied/unzipped from
# the parent directory
        @fntodelete = ();               # possible files to unzip/delete
        for ($n = 0; $n < $nfile; $n++) {
                if (! -f $mrgfiles[$n]) {
                        @fntodelete = (@fntodelete, &copyunzip("$abinit_inpdir/$TestDir/Input/$mrgfiles[$n]")); # copy/unzip
                        }
                }
        $fntodelete = &copyunzip("$abinit_inpdir/$TestDir/Input/$fntounzip") if ($fntounzip ne '') ;    # copy/unzip if any
# Create "files" file with file names $fn1, ... one per line
        open(FILES,">$RUNfile") || die "Unable to open FILES for test $TID";
        print FILES $nfile;     # put files count
        print FILES $outscrfn;     # translate path name when required
        for ($n = 0; $n < $nfile; $n++) {
                print FILES &transpath("$mrgfiles[$n]");        # translate path name when required
                }
        close(FILES);
        return if ($debug > 2);
# Run the test case
        print "File $RUNfile created, starting mrgscr";
        if ($OStype eq 'MacOS') {
                system ("$CODE_MRGSCR < $RUNfile \xb7 $outfn ");
                }
        else {
                system ("$CODE_MRGSCR < $RUNfile > $outfn $REDIRECT_ERR");
                }
# Compare with reference files
        $REFoutfn = &transpath("$REF/$outfn");
        print "Finished mrgscr, comparing $outfn with $REFoutfn";
        system ("$DIFF_COMMAND $outfn $REFoutfn > $difffn");
...
#	for cases v2 #80, v3 #59,61,63,64,66,67 set .log file to .out (not the usual log file)
	...
	unlink($logfn,$difffn,$outfn,$outdenfn);
	if ($fn3 ne '') {
...
# Compare with reference files
	$REFoutfn = &transpath("$REF/$fn2");
	print "Finished cut3d, comparing $outfn with $REFoutfn";
	system ("$DIFF_COMMAND $outfn $REFoutfn > $difffn");
	&dofldiff("Case_$TID","$fn2","$REF/$fn2",$fldopt);  # compare floating point numbers
	return;			# go read next line from configuration file
	}
...
# the parameter format for an option is: opt=option ; see setfnopt
# set default files names derived from $TID:
	$fn1 = "$top_testdir/$TestDir/Input/t$TID.in";	# e.g. ../t57.in
...
# Create "files" file with file names $fn1, ... one per line
	...
	return if ($debug >= 3);
# Run the test case
	print "File $RUNfile created, starting aim";
	if ($OStype eq 'MacOS') {
...
# Compare with reference files
	$REFoutfn = &transpath("$REF/$outfn");
	print "Finished aim, comparing $outfn with $REFoutfn";
	system ("$DIFF_COMMAND $outfn $REFoutfn > $difffn");
	&dofldiff("Case_$TID","$outfn","$REF/$outfn",$fldopt);  # compare floating point numbers
	return;              # go read next line from configuration file
	}
...
# Compare with reference files
	$REFoutfn = &transpath("$REF/$fn2");
	print "Finished conducti, comparing $outfn with $REFoutfn";
	system ("$DIFF_COMMAND $outfn $REFoutfn > $difffn");
	&dofldiff("Case_$TID","$fn2","$REF/$fn2",$fldopt);  # compare floating point numbers
	return;                 # go read next line from configuration file
	}
...
# Remove existing files related to this test case:
        ...
        unlink($RUNfile,$difffn,$logfn,$outfnl,$outfnr,$outfni);
...
# Compare with reference files
        ...
        print "Finished optic, comparing $outfnl with $REFoutfn";
        ...
        if(-f $REFoutfn) {
          print "Finished optic, comparing $outfnr with $REFoutfn";
        ...
        if(-f $REFoutfn) {
          print "Finished optic, comparing $outfni with $REFoutfn";
...
# set default files names derived from $TID:
	$fn1 = "$top_testdir/$TestDir/Input/t$TID.in";		# e.g. ../t88.in
	$fn2 = "$top_testdir/$TestDir/Input/t$TID.moldyn.in";	# ../t88.moldyn.in
...
# Remove existing files related to this test case:
	...
	unlink($RUNfile,$logfn,$difffn,$outfn,$wandatafn);
# Create "files" file with file names $fn1, ... one per line
	open(FILES,">$RUNfile") || die "Unable to open FILES for test $TID";
	print FILES &transpath($fn1);   # translate path name when required
...
# Compare with reference files
	...
	print "Finished lwf, comparing $outfn with $REFoutfn";
	system ("$DIFF_COMMAND $outfn $REFoutfn > $difffn");
...
#	$fn4 = _displ file
# set default files names derived from $TID:
	$fn1 = "$top_testdir/$TestDir/Input/t$TID.in";		# e.g. ../t51.in
	$fn2 = "t$TID.out.eps";			# t51.out.eps
	$fn3 = "$top_testdir/$TestDir/Input/t$TID.in_freq";	# ../t51.in_freq
	$fn4 = "$top_testdir/$TestDir/Input/t$TID.in_displ";	# ../t51.in_displ
# set new file names or options according to the parameters
	for ($i=1;$i <= 5;$i++) {
		last if (@_[$i] eq '');
		&setfnopt(@_[$i]);
...
# Remove existing files related to this test case:
	...
	unlink($RUNfile,$logfn,$difffn,$epsfn);
# Create "files" file with file names $fn1, ... one per line  
	open(FILES,">$RUNfile") || die "Unable to open FILES for test $TID";
	...
	return if ($debug >= 3);
# Run the test case
	print "File $RUNfile created, starting band2eps";
	if ($OStype eq 'MacOS') {
...
# Compare with reference files
	...
	print "Finished band2eps, comparing $epsfn with $REFepsfn";
...
# Create "macroave.in" file from file $fn1
	...
	print "Case_$TID: file $fn1 copied to $RUNfile" if ($rc1 == 0);

	return if ($debug >= 3);
# Run the test case
	print "Starting macroave";
	system ("$CODE_MACROAVE");

# Compare with reference files
	$REFoutfn = &transpath("$REF/$fn2");
	print "Finished macroave, comparing $outfn1 with $REFoutfn";
	system ("$DIFF_COMMAND $outfn1 $REFoutfn > $difffn");
	&dofldiff("Case_$TID","$fn2","$REF/$fn2",$fldopt);  # compare floating point numbers
	$REFoutfn = &transpath("$REF/$fn3");
	print "Finished macroave, comparing $outfn2 with $REFoutfn";
	system ("$DIFF_COMMAND $outfn1 $REFoutfn > $difffn");
	&dofldiff("Case_$TID","$fn3","$REF/$fn3",$fldopt);  # compare floating point numbers
	return;                 # go read next line from configuration file
	}
...
sub doserieX {
	local($TID,$p1,$p2,$p3,$p4,$p5) = @_;
#
# purpose: run cpu tests of the sequential version of abinis
#
# arguments:
...
# Change to the test subdirectory
  ...
  $WKdir = &transpath("$WORK_DIR/$SERIE_DIR");
	...
	$RUNfile = 'filnames.run';	# files file for abinis
...
# copy "files" file from upper directory and translate file pathes
	...
	open(FILES,">$RUNfile") || die "Unable to open FILES for test $TID";
	$inputfn = <FILEI>;	# first line from files is also input file for abinis
...
# DEFAULT NAMES for the files, input, log, ... files for abinis and getrep
	$FNinput = 'input';
	$LOGFILE = 'log';
	$FNout= 'output';	# abinis output file
...
#		$P2 = destination file or directory
	...
	$rc = system($cmd);
	if ($rc == 0) {
		print "Case_$TID: file $p1 copied";
		}
	else {
		print "Error $rc copying $p1";
		}
	return;			# go read next line from configuration file
	}
# ****************************************
...
#		$p1 = name of file(s) to be erased
	...
	$rc = system($cmd);
	if ($rc == 0) {
		print "Case_$TID: file $p1 erased";
		}
	else {
		print "Error $rc erasing $p1";
		}
	return;			# go read next line from configuration file
	}
# ****************************************
...
#       Read both files, one line at a time
	...
	while (1) {
	  ...
	  if ($field1[3] eq "fatal") {
	    ...
	    $tolabs = $field2[4] ;
            $tolrel = $field2[6] ;
	    if ( $field1[10] > $tolabs*1.5 ) {
              print FILEOUT "$field1[1]            failed (too large absolute error : $field1[10] , accepted $tolabs ) " ;
              }
            ...
            elsif ( $field1[10] > $tolabs ) {
              print FILEOUT "$field1[1] within 1.5 of tolerance (absolute error : $field1[10] , accepted $tolabs ) " ;
              }
