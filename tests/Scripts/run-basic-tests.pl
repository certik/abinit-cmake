...
# Copyright (C) 1999-2008 ABINIT group (LSi,XG)
...
#
$DELAY = 2;		# delay in seconds between 2 verifications of the STATUS file
$MAXLOOP = '50';	# maximum count for STATUS file checking loop
%PspFiles=('1','01h.pspgth',
	 '2','14si.pspgth',
	 '3','01h.pspgth:04be.pspgth',
	 '4','70yb.pspnc',
	 '5','13al.pspgth',
	 '6','14si.xml');
#
...
if ($ARGV[1] eq '') {
  print 'Missing argument';
	print "Usage is: run-basic-tests machine_name [ 1 | 2 | 3 | 4 | 5 | 6] srcDir";
	exit 16;
	}
$MACH = $ARGV[0];
$TestN = $ARGV[1];
...
$inputfile = &transpath("$srcDir/tests/built-in/Input/test$TestN.in");
...
system("$COPY_COMMAND ".&transpath("$srcDir/tests/built-in/Input/test$TestN.in")." test$TestN.in");
...
print FILES &transpath("$srcDir/tests/Psps_for_tests/$PsP[0]");
print FILES &transpath("$srcDir/tests/Psps_for_tests/$PsP[1]") if ($PsP[1] ne '');
close (FILES);
# abinis will be started through a dotest perl script
# this is necessary under DOS/Windows because of file redirections
$cmd = $PRLPFX.&transpath("$srcDir/tests/Scripts/run-basic-dotest$PLSUFX")." $TestN $ENDtest $SLASH $SUFXstyle $BGSUFX";
