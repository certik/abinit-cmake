...
! Write general rules
  ...
  write( 2 , '( a )' ) '	/include=([--.src.defs]) $(MMS$TARGET_NAME).F'
  write( 2 , '( a )' ) '	f90/source=fixed/convert=big_endian/name=lowercase\'
  write( 2 , '( a )' ) '	/opti=(tune=host,nopipe)/arch=host/debug/include=([--.src.defs],[--],sys$common:[syslib])\'
  write( 2 , '( a )' ) '	/check=(noarg,bo,form,fp,noout,over,powe,under) $(MMS$TARGET_NAME).f_'
  write( 2 , '( a )' ) '	delete $(MMS$TARGET_NAME).f_;*'
  write( 2 , '( )' )
  write( 2 , '( a )' ) '.f90.obj :'
  write( 2 , '( a )' ) '	cc/define=(VMS,HAVE_NETCDF,HAVE_XMLF90)/comment=as_is/prep=$(MMS$TARGET_NAME).f_\'
  write( 2 , '( a )' ) '	/include=([--.src.defs]) $(MMS$TARGET_NAME).F90'
